...
{
    cvector_clean(TASKS_NODE_CSOCKET_CNODE_VEC(tasks_node),
                (CVECTOR_DATA_CLEANER)csocket_cnode_close_and_clean_event, LOC_TASKS_0005);
...
void tasks_node_print(LOG *log, const TASKS_NODE *tasks_node)
{
    ...
    CVECTOR_LOCK(csocket_cnode_vec, LOC_TASKS_0013);
    ...
    CVECTOR_UNLOCK(csocket_cnode_vec, LOC_TASKS_0014);
    return ;
}
...
void tasks_node_print_csocket_cnode_list(LOG *log, const TASKS_NODE *tasks_node, UINT32 *index)
{
    ...
    csocket_cnode_vec = (CVECTOR *)TASKS_NODE_CSOCKET_CNODE_VEC(tasks_node);
    CVECTOR_LOCK(csocket_cnode_vec, LOC_TASKS_0015);
    for(pos = 0; pos < cvector_size(csocket_cnode_vec); pos ++)
    {
        CSOCKET_CNODE *csocket_cnode;
    ...
    CVECTOR_UNLOCK(csocket_cnode_vec, LOC_TASKS_0016);
    return ;
}
...
UINT32 tasks_worker_count(const TASKS_WORKER *tasks_worker, const UINT32 tcid, const UINT32 srv_ipaddr, const UINT32 srv_port)
{
    ...
    CVECTOR_LOCK(TASKS_WORKER_NODES(tasks_worker), LOC_TASKS_0021);
    count = tasks_worker_count_no_lock(tasks_worker, tcid, srv_ipaddr, srv_port);
    CVECTOR_UNLOCK(TASKS_WORKER_NODES(tasks_worker), LOC_TASKS_0022);
...
TASKS_NODE *tasks_worker_search_tasks_node_by_ipaddr(const TASKS_WORKER *tasks_worker, const UINT32 ipaddr)
{
    ...
    CVECTOR_LOCK(tasks_nodes, LOC_TASKS_0023);
    for(pos = 0; pos < cvector_size(tasks_nodes); pos ++)
    {
        ...
        {
            CVECTOR_UNLOCK(tasks_nodes, LOC_TASKS_0024);
            return (tasks_node);
        }
    }
...
TASKS_NODE *tasks_worker_search_tasks_node_by_tcid(const TASKS_WORKER *tasks_worker, const UINT32 tcid)
{
    ...
    CVECTOR_LOCK(tasks_nodes, LOC_TASKS_0026);
    for(pos = 0; pos < cvector_size(tasks_nodes); pos ++)
    {
        TASKS_NODE *tasks_node;
        ...
        if(tcid == TASKS_NODE_TCID(tasks_node))
        {
            CVECTOR_UNLOCK(tasks_nodes, LOC_TASKS_0027);
            return (tasks_node);
        }
    }
...
TASKS_NODE *tasks_worker_search_tasks_node_by_tcid_comm(const TASKS_WORKER *tasks_worker, const UINT32 tcid, const UINT32 comm)
{
    ...
    CVECTOR_LOCK(tasks_nodes, LOC_TASKS_0029);
    for(pos = 0; pos < cvector_size(tasks_nodes); pos ++)
    {
        TASKS_NODE *tasks_node;
        ...
        {
            CVECTOR_UNLOCK(tasks_nodes, LOC_TASKS_0030);
            return (tasks_node);
        }
    }
...
EC_BOOL tasks_worker_delete(TASKS_WORKER *tasks_worker, const UINT32 tcid, const UINT32 ipaddr, const UINT32 port)
{
    ...
    tasks_node = tasks_worker_search(tasks_worker, tcid, ipaddr, port);
    if(NULL_PTR == tasks_node)
    {
        dbg_log(SEC_0121_TASKS, 0)(LOGSTDOUT, "[DEBUG] tasks_worker_delete: "
                                              "no tasks_node of tcid %s ip %s, port %ld\n",
                                              c_word_to_ipv4(tcid),
                                              c_word_to_ipv4(ipaddr),
                                              port);
        return (EC_TRUE);
    ...
    while(NULL_PTR != (csocket_cnode = cvector_pop(TASKS_NODE_CSOCKET_CNODE_VEC(tasks_node))))
    {
        csocket_cnode_iclose(csocket_cnode);
    }
    ...
    {
        dbg_log(SEC_0121_TASKS, 0)(LOGSTDOUT, "[DEBUG] tasks_worker_delete: "
                                              "tcid %s comm %ld has no connections\n",
                                              TASKS_NODE_TCID_STR(tasks_node),
                                              TASKS_NODE_COMM(tasks_node));
...
{
    cvector_init(TASKS_WORKER_NODES(tasks_worker), 0, MM_TASKS_NODE, CVECTOR_LOCK_ENABLE, LOC_TASKS_0036);
...
void tasks_worker_print(LOG *log, const TASKS_WORKER *tasks_worker)
{
    ...
    cvector_print(log, TASKS_WORKER_NODES(tasks_worker),
                                (CVECTOR_DATA_PRINT)tasks_node_print);
...
EC_BOOL tasks_worker_isend_node(TASKS_WORKER *tasks_worker, const UINT32 des_tcid, const UINT32 des_comm, const UINT32 msg_tag, TASK_NODE *task_node)
{
    ...
    if(NULL_PTR == tasks_node)
    {
        ...
        && CMPI_LOCAL_TCID == des_tcid)
        {
            super_add_connection(0 /*super_md_id*/, des_tcid, des_comm,
                             TASKS_CFG_SRVIPADDR(remote_tasks_cfg), TASKS_CFG_SRVPORT(remote_tasks_cfg),
                             (UINT32)CSOCKET_CNODE_NUM);
            return (EC_FALSE);
        }
...
TASKS_NODE *tasks_monitor_search_tasks_node_by_tcid(const TASKS_MONITOR *tasks_monitor, const UINT32 tcid)
{
    ...
    CVECTOR_LOCK(tasks_nodes, LOC_TASKS_0042);
    for(pos = 0; pos < cvector_size(tasks_nodes); pos ++)
    {
        TASKS_NODE *tasks_node;
...
UINT32 tasks_monitor_count(const TASKS_MONITOR *tasks_monitor, const UINT32 tcid, const UINT32 srv_ipaddr, const UINT32 srv_port)
{
    ...
    CVECTOR_LOCK(TASKS_MONITOR_NODES(tasks_monitor), LOC_TASKS_0045);
    count = tasks_monitor_count_no_lock(tasks_monitor, tcid, srv_ipaddr, srv_port);
    CVECTOR_UNLOCK(TASKS_MONITOR_NODES(tasks_monitor), LOC_TASKS_0046);
...
EC_BOOL tasks_monitor_open(TASKS_MONITOR *tasks_monitor, const UINT32 tcid, const UINT32 srv_ipaddr, const UINT32 srv_port)
{
    ...
    csocket_cnode = csocket_cnode_new(LOC_TASKS_0047);/*client save remote server ipaddr and srvport info*/
    if(NULL_PTR == csocket_cnode)
    {
...
{
    cvector_init(TASKS_MONITOR_NODES(tasks_monitor), 0, MM_TASKS_NODE, CVECTOR_LOCK_ENABLE, LOC_TASKS_0048);
...
void tasks_monitor_print_csocket_cnode_list_in_plain(LOG *log, const TASKS_MONITOR *tasks_monitor, UINT32 *index)
{
    const CVECTOR      *tasks_nodes;
    UINT32              pos;
    UINT32              num;
...
void tasks_monitor_print_in_plain(LOG *log, const TASKS_MONITOR *tasks_monitor)
{
    ...
    index = 0;
    tasks_monitor_print_csocket_cnode_list_in_plain(log, tasks_monitor, &index);
...
STATIC_CAST static TASK_NODE *__tasks_handshake_encode()
{
    ...
    task_node = task_node_new(data_num, LOC_TASKS_0052);
    if(NULL_PTR == task_node)
    {
