...
EC_BOOL crfshttp_log_start()
{
    ...
    if(EC_TRUE == task_brd_check_is_work_tcid(TASK_BRD_TCID(task_brd)))
    {
        ...
        {
            dbg_log(SEC_0049_CRFSHTTP, 0)(LOGSTDOUT, "error:crfshttp_log_start: user_log_open '%s' -> LOGUSER08 failed\n",
    ...
    if(EC_TRUE == task_brd_check_is_work_tcid(TASK_BRD_TCID(task_brd)))
    {
        ...
        if(NULL_PTR == log)
        {
            dbg_log(SEC_0049_CRFSHTTP, 0)(LOGSTDOUT, "error:crfshttp_log_start: log_file_open '%s' -> LOGUSER08 failed\n",
                               (char *)cstring_get_str(log_file_name));
            cstring_free(log_file_name);
            /*task_brd_default_abort();*/
        }
        else
        {
            ...
            dbg_log(SEC_0049_CRFSHTTP, 0)(LOGSTDOUT, "[DEBUG] crfshttp_log_start: log_file_open '%s' -> LOGUSER08 done\n",
                               (char *)cstring_get_str(log_file_name));
    ...
    if(EC_TRUE == task_brd_check_is_work_tcid(TASK_BRD_TCID(task_brd)))
    {
        ...
        if(NULL_PTR == log)
        {
            dbg_log(SEC_0049_CRFSHTTP, 0)(LOGSTDOUT, "error:crfshttp_log_start: log_file_open '%s' -> LOGUSER07 failed\n",
                               (char *)cstring_get_str(log_file_name));
            cstring_free(log_file_name);
            /*task_brd_default_abort();*/
        }
...
STATIC_CAST static UINT32 __crfshttp_convert_expires_str_to_nseconds(const char *expires_str)
{
    char *str;
    char *fields[2];
    UINT32 seg_num;
...
STATIC_CAST static EC_BOOL __crfshttp_uri_is_file_wait_get_op(const CBUFFER *uri_cbuffer)
{
    const uint8_t *uri_str;
    uint32_t       uri_len;
...
EC_BOOL crfshttp_commit_file_terminate_get_request(CHTTP_NODE *chttp_node)
{
    ...
    {
        dbg_log(SEC_0049_CRFSHTTP, 0)(LOGSTDOUT, "error:crfshttp_commit_file_terminate_get_request: handle 'GET' request failed\n");
        return (EC_FALSE);
    }

    if(EC_FALSE == crfshttp_make_file_terminate_get_response(chttp_node))
    {
        dbg_log(SEC_0049_CRFSHTTP, 0)(LOGSTDOUT, "error:crfshttp_commit_file_terminate_get_request: make 'GET' response failed\n");
        return (EC_FALSE);
    }
    ...
    if(EC_FALSE == ret)
    {
        dbg_log(SEC_0049_CRFSHTTP, 0)(LOGSTDOUT, "error:crfshttp_commit_file_terminate_get_request: commit 'GET' response failed\n");
        return (EC_FALSE);
    }
...
EC_BOOL crfshttp_commit_cond_terminate_get_request(CHTTP_NODE *chttp_node)
{
    ...
    {
        dbg_log(SEC_0049_CRFSHTTP, 0)(LOGSTDOUT, "error:crfshttp_commit_cond_terminate_get_request: handle 'GET' request failed\n");
        return (EC_FALSE);
    }

    if(EC_FALSE == crfshttp_make_cond_terminate_get_response(chttp_node))
    {
        dbg_log(SEC_0049_CRFSHTTP, 0)(LOGSTDOUT, "error:crfshttp_commit_cond_terminate_get_request: make 'GET' response failed\n");
        return (EC_FALSE);
    }
    ...
    if(EC_FALSE == ret)
    {
        dbg_log(SEC_0049_CRFSHTTP, 0)(LOGSTDOUT, "error:crfshttp_commit_cond_terminate_get_request: commit 'GET' response failed\n");
        return (EC_FALSE);
    }
...
STATIC_CAST static EC_BOOL __crfshttp_uri_is_wait_header_get_op(const CBUFFER *uri_cbuffer)
{
    const uint8_t *uri_str;
    uint32_t       uri_len;
...
STATIC_CAST static EC_BOOL __crfshttp_uri_is_rfs_up_get_op(const CBUFFER *uri_cbuffer)
{
    const uint8_t *uri_str;
    uint32_t       uri_len;
...
EC_BOOL crfshttp_handle_rfs_up_get_request(CHTTP_NODE *chttp_node)
{
    ...
    if(EC_TRUE == __crfshttp_uri_is_rfs_up_get_op(uri_cbuffer))
    {
        ...
        if(CMPI_ERROR_MODI == cmon_id)
        {
            ...
            CHTTP_NODE_LOG_INFO_WHEN_DONE(chttp_node, "error:crfshttp_handle_rfs_up_get_request: no cmon start");
        ...
        if(EC_FALSE == cmon_set_node_up(cmon_id, &cmon_node))
        {
            ...
            CHTTP_NODE_LOG_INFO_WHEN_DONE(chttp_node, "error:crfshttp_handle_rfs_up_get_request: set up rfs %s failed", c_word_to_ipv4(CMON_NODE_TCID(&cmon_node)));
        ...
        CHTTP_NODE_LOG_INFO_WHEN_DONE(chttp_node, "[DEBUG] crfshttp_handle_rfs_up_get_request: set up rfs %s done", c_word_to_ipv4(CMON_NODE_TCID(&cmon_node)));
...
STATIC_CAST static EC_BOOL __crfshttp_uri_is_rfs_down_get_op(const CBUFFER *uri_cbuffer)
{
    const uint8_t *uri_str;
    uint32_t       uri_len;
...
EC_BOOL crfshttp_handle_rfs_down_get_request(CHTTP_NODE *chttp_node)
{
    ...
    if(EC_TRUE == __crfshttp_uri_is_rfs_down_get_op(uri_cbuffer))
    {
        ...
        if(CMPI_ERROR_MODI == cmon_id)
        {
            ...
            CHTTP_NODE_LOG_TIME_WHEN_DONE(chttp_node);
            CHTTP_NODE_LOG_STAT_WHEN_DONE(chttp_node, "RFSMON_FAIL %s %u --", "GET", CHTTP_NOT_IMPLEMENTED);
            CHTTP_NODE_LOG_INFO_WHEN_DONE(chttp_node, "error:crfshttp_handle_rfs_down_get_request: no cmon start");
        ...
        if(EC_FALSE == cmon_set_node_down(cmon_id, &cmon_node))
        {
            ...
            CHTTP_NODE_LOG_TIME_WHEN_DONE(chttp_node);
            CHTTP_NODE_LOG_STAT_WHEN_DONE(chttp_node, "RFSMON_FAIL %s %u --", "GET", CHTTP_FORBIDDEN);
            CHTTP_NODE_LOG_INFO_WHEN_DONE(chttp_node, "error:crfshttp_handle_rfs_down_get_request: set down rfs %s failed", c_word_to_ipv4(CMON_NODE_TCID(&cmon_node)));
        ...
        CHTTP_NODE_LOG_TIME_WHEN_DONE(chttp_node);
        CHTTP_NODE_LOG_STAT_WHEN_DONE(chttp_node, "RFSMON_SUCC %s %u --", "GET", CHTTP_OK);
        CHTTP_NODE_LOG_INFO_WHEN_DONE(chttp_node, "[DEBUG] crfshttp_handle_rfs_down_get_request: set down rfs %s done", c_word_to_ipv4(CMON_NODE_TCID(&cmon_node)));
...
STATIC_CAST static EC_BOOL __crfshttp_uri_is_rfs_add_get_op(const CBUFFER *uri_cbuffer)
{
    const uint8_t *uri_str;
    uint32_t       uri_len;
...
EC_BOOL crfshttp_handle_rfs_add_get_request(CHTTP_NODE *chttp_node)
{
    ...
    if(EC_TRUE == __crfshttp_uri_is_rfs_add_get_op(uri_cbuffer))
    {
        ...
        if(CMPI_ERROR_MODI == cmon_id)
        {
            ...
            CHTTP_NODE_LOG_TIME_WHEN_DONE(chttp_node);
            CHTTP_NODE_LOG_STAT_WHEN_DONE(chttp_node, "RFSMON_FAIL %s %u --", "GET", CHTTP_NOT_IMPLEMENTED);
            CHTTP_NODE_LOG_INFO_WHEN_DONE(chttp_node, "error:crfshttp_handle_rfs_add_get_request: no cmon start");
        ...
        if(EC_FALSE == cmon_add_node(cmon_id, &cmon_node))
        {
            ...
            CHTTP_NODE_LOG_TIME_WHEN_DONE(chttp_node);
            CHTTP_NODE_LOG_STAT_WHEN_DONE(chttp_node, "RFSMON_FAIL %s %u --", "GET", CHTTP_FORBIDDEN);
            CHTTP_NODE_LOG_INFO_WHEN_DONE(chttp_node, "error:crfshttp_handle_rfs_add_get_request: add rfs %s failed", c_word_to_ipv4(CMON_NODE_TCID(&cmon_node)));
        ...
        CHTTP_NODE_LOG_TIME_WHEN_DONE(chttp_node);
        CHTTP_NODE_LOG_STAT_WHEN_DONE(chttp_node, "RFSMON_SUCC %s %u --", "GET", CHTTP_OK);
        CHTTP_NODE_LOG_INFO_WHEN_DONE(chttp_node, "[DEBUG] crfshttp_handle_rfs_add_get_request: add rfs %s done", c_word_to_ipv4(CMON_NODE_TCID(&cmon_node)));
...
STATIC_CAST static EC_BOOL __crfshttp_uri_is_rfs_del_get_op(const CBUFFER *uri_cbuffer)
{
    const uint8_t *uri_str;
    uint32_t       uri_len;
...
EC_BOOL crfshttp_handle_rfs_del_get_request(CHTTP_NODE *chttp_node)
{
    ...
    if(EC_TRUE == __crfshttp_uri_is_rfs_del_get_op(uri_cbuffer))
    {
        ...
        if(CMPI_ERROR_MODI == cmon_id)
        {
            ...
            CHTTP_NODE_LOG_TIME_WHEN_DONE(chttp_node);
            CHTTP_NODE_LOG_STAT_WHEN_DONE(chttp_node, "RFSMON_FAIL %s %u --", "GET", CHTTP_NOT_IMPLEMENTED);
            CHTTP_NODE_LOG_INFO_WHEN_DONE(chttp_node, "error:crfshttp_handle_rfs_del_get_request: no cmon start");
        ...
        if(EC_FALSE == cmon_del_node(cmon_id, &cmon_node))
        {
            ...
            CHTTP_NODE_LOG_TIME_WHEN_DONE(chttp_node);
            CHTTP_NODE_LOG_STAT_WHEN_DONE(chttp_node, "RFSMON_FAIL %s %u --", "GET", CHTTP_FORBIDDEN);
            CHTTP_NODE_LOG_INFO_WHEN_DONE(chttp_node, "error:crfshttp_handle_rfs_del_get_request: del rfs %s failed", c_word_to_ipv4(CMON_NODE_TCID(&cmon_node)));
        ...
        CHTTP_NODE_LOG_TIME_WHEN_DONE(chttp_node);
        CHTTP_NODE_LOG_STAT_WHEN_DONE(chttp_node, "RFSMON_SUCC %s %u --", "GET", CHTTP_OK);
        CHTTP_NODE_LOG_INFO_WHEN_DONE(chttp_node, "[DEBUG] crfshttp_handle_rfs_del_get_request: del rfs %s done", c_word_to_ipv4(CMON_NODE_TCID(&cmon_node)));
...
STATIC_CAST static EC_BOOL __crfshttp_uri_is_rfs_list_get_op(const CBUFFER *uri_cbuffer)
{
    const uint8_t *uri_str;
    uint32_t       uri_len;
...
EC_BOOL crfshttp_handle_rfs_list_get_request(CHTTP_NODE *chttp_node)
{
    ...
    if(EC_TRUE == __crfshttp_uri_is_rfs_list_get_op(uri_cbuffer))
    {
        ...
        if(CMPI_ERROR_MODI == cmon_id)
        {
            ...
            CHTTP_NODE_LOG_TIME_WHEN_DONE(chttp_node);
            CHTTP_NODE_LOG_STAT_WHEN_DONE(chttp_node, "RFSMON_FAIL %s %u --", "GET", CHTTP_NOT_IMPLEMENTED);
            CHTTP_NODE_LOG_INFO_WHEN_DONE(chttp_node, "error:crfshttp_handle_rfs_list_get_request: no cmon start");
...
STATIC_CAST static EC_BOOL __crfshttp_uri_is_ngx_breathe_get_op(const CBUFFER *uri_cbuffer)
{
    const uint8_t *uri_str;
    uint32_t       uri_len;
...
EC_BOOL crfshttp_is_http_get_ngx_breathe(const CHTTP_NODE *chttp_node)
{
    ...
    dbg_log(SEC_0049_CRFSHTTP, 9)(LOGSTDOUT, "[DEBUG] crfshttp_is_http_get_ngx_breathe: uri: '%.*s' [len %d]\n",
                        CBUFFER_USED(uri_cbuffer),
                        CBUFFER_DATA(uri_cbuffer),
                        CBUFFER_USED(uri_cbuffer));

    if(EC_TRUE == __crfshttp_uri_is_ngx_breathe_get_op(uri_cbuffer))
    {
        return (EC_TRUE);
    }
...
EC_BOOL crfshttp_commit_ngx_breathe_get_request(CHTTP_NODE *chttp_node)
{
    ...
    if(EC_FALSE == crfshttp_handle_ngx_breathe_get_request(chttp_node))
    {
        dbg_log(SEC_0049_CRFSHTTP, 0)(LOGSTDOUT, "error:crfshttp_commit_ngx_breathe_get_request: handle 'GET' request failed\n");
        return (EC_FALSE);
    }

    if(EC_FALSE == crfshttp_make_ngx_breathe_get_response(chttp_node))
    {
        dbg_log(SEC_0049_CRFSHTTP, 0)(LOGSTDOUT, "error:crfshttp_commit_ngx_breathe_get_request: make 'GET' response failed\n");
        return (EC_FALSE);
    }

    ret = crfshttp_commit_ngx_breathe_get_response(chttp_node);
    if(EC_FALSE == ret)
    {
        dbg_log(SEC_0049_CRFSHTTP, 0)(LOGSTDOUT, "error:crfshttp_commit_ngx_breathe_get_request: commit 'GET' response failed\n");
        return (EC_FALSE);
    }
...
EC_BOOL crfshttp_handle_ngx_breathe_get_request(CHTTP_NODE *chttp_node)
{
    ...
    if(!(0 == req_body_chunk_num))
    {
        ...
        dbg_log(SEC_0049_CRFSHTTP, 0)(LOGSTDOUT, "error: crfshttp_handle_ngx_breathe_get_request: chunk num %ld\n", req_body_chunk_num);

        dbg_log(SEC_0049_CRFSHTTP, 0)(LOGSTDOUT, "error: crfshttp_handle_ngx_breathe_get_request: chunk mgr %p info\n", req_body_chunks);
        chunk_mgr_print_info(LOGSTDOUT, req_body_chunks);

        dbg_log(SEC_0049_CRFSHTTP, 0)(LOGSTDOUT, "error: crfshttp_handle_ngx_breathe_get_request: chunk mgr %p chars\n", req_body_chunks);
        chunk_mgr_print_chars(LOGSTDOUT, req_body_chunks);

        CHTTP_NODE_LOG_TIME_WHEN_DONE(chttp_node);
        CHTTP_NODE_LOG_STAT_WHEN_DONE(chttp_node, "RFS_ERR %s %u --", "GET", CHTTP_BAD_REQUEST);
        CHTTP_NODE_LOG_INFO_WHEN_DONE(chttp_node, "[DEBUG] crfshttp_handle_ngx_breathe_get_request");
...
EC_BOOL crfshttp_make_ngx_breathe_get_response(CHTTP_NODE *chttp_node)
{
    if(EC_FALSE == chttp_make_response_header_common(chttp_node, (uint64_t)0))
    {
        dbg_log(SEC_0049_CRFSHTTP, 0)(LOGSTDOUT, "error:crfshttp_make_ngx_breathe_get_response: make response header failed\n");
        return (EC_FALSE);
    }
    ...
    if(BIT_TRUE == CHTTP_NODE_KEEPALIVE(chttp_node))
    {
        if(EC_FALSE == chttp_make_response_header_keepalive(chttp_node))
        {
            dbg_log(SEC_0049_CRFSHTTP, 0)(LOGSTDOUT, "error:crfshttp_make_ngx_breathe_get_response: make response header keepalive failed\n");
            return (EC_FALSE);
        }
    }
    ...
    if(EC_FALSE == chttp_make_response_header_end(chttp_node))
    {
        dbg_log(SEC_0049_CRFSHTTP, 0)(LOGSTDOUT, "error:crfshttp_make_ngx_breathe_get_response: make header end failed\n");
        return (EC_FALSE);
    }
...
EC_BOOL crfshttp_commit_ngx_breathe_get_response(CHTTP_NODE *chttp_node)
{
    ...
    if(NULL_PTR == csocket_cnode)
    {
        dbg_log(SEC_0049_CRFSHTTP, 0)(LOGSTDOUT, "error:crfshttp_commit_ngx_breathe_get_response: csocket_cnode of chttp_node %p is null\n", chttp_node);
        return (EC_FALSE);
    }
