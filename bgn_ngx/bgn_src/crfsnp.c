...
CRFSNP_KEY *crfsnp_key_new()
{
    ...
    alloc_static_mem(MM_CRFSNP_KEY, &crfsnp_key, LOC_CRFSNP_0007);
...
{
    CRFSNP_KEY_LEN(crfsnp_key) = 0;
    BSET(CRFSNP_KEY_NAME(crfsnp_key), '\0', CRFSNP_KEY_MAX_SIZE);
    return (EC_TRUE);
}
...
EC_BOOL crfsnp_key_clean(CRFSNP_KEY *crfsnp_key)
{
    ...
    CRFSNP_KEY_NAME(crfsnp_key)[ 0 ] = '\0';
...
{
    if(NULL_PTR == crfsnp_key_src)
    {
        dbg_log(SEC_0081_CRFSNP, 0)(LOGSTDOUT, "error:crfsnp_key_clone: crfsnp_key_src is null\n");
        return (EC_FALSE);
    }

    if(NULL_PTR == crfsnp_key_des)
    {
        dbg_log(SEC_0081_CRFSNP, 0)(LOGSTDOUT, "error:crfsnp_key_clone: crfsnp_key_des is null\n");
        return (EC_FALSE);
    }

    CRFSNP_KEY_LEN(crfsnp_key_des) =  CRFSNP_KEY_LEN(crfsnp_key_src);

    BCOPY(CRFSNP_KEY_NAME(crfsnp_key_src), CRFSNP_KEY_NAME(crfsnp_key_des), CRFSNP_KEY_LEN(crfsnp_key_src));
    return (EC_TRUE);
}
...
EC_BOOL crfsnp_key_free(CRFSNP_KEY *crfsnp_key)
{
    if(NULL_PTR != crfsnp_key)
    {
        crfsnp_key_clean(crfsnp_key);
        free_static_mem(MM_CRFSNP_KEY, crfsnp_key, LOC_CRFSNP_0008);
    }
    return (EC_TRUE);
}
...
{
    BCOPY(key, CRFSNP_KEY_NAME(crfsnp_key), klen);
    CRFSNP_KEY_LEN(crfsnp_key) = (uint8_t)klen;
...
void crfsnp_key_print(LOG *log, const CRFSNP_KEY *crfsnp_key)
{
    sys_log(log, "klen: %u, key: %.*s\n",
                 CRFSNP_KEY_LEN(crfsnp_key),
                 CRFSNP_KEY_LEN(crfsnp_key), CRFSNP_KEY_NAME(crfsnp_key));
...
EC_BOOL crfsnp_item_clone(const CRFSNP_ITEM *crfsnp_item_src, CRFSNP_ITEM *crfsnp_item_des)
{
    ...
    CRFSNP_ITEM_KEY_OFFSET(crfsnp_item_des)  = CRFSNP_ITEM_KEY_OFFSET(crfsnp_item_src);
    CRFSNP_ITEM_PARENT_POS(crfsnp_item_des)  = CRFSNP_ITEM_PARENT_POS(crfsnp_item_src);
    CRFSNP_ITEM_SECOND_HASH(crfsnp_item_des) = CRFSNP_ITEM_SECOND_HASH(crfsnp_item_src);

    crfsnplru_node_clone(CRFSNP_ITEM_LRU_NODE(crfsnp_item_src), CRFSNP_ITEM_LRU_NODE(crfsnp_item_des));
    crfsnpdel_node_clone(CRFSNP_ITEM_DEL_NODE(crfsnp_item_src), CRFSNP_ITEM_DEL_NODE(crfsnp_item_des));
...
{
    BCOPY(key, CRFSNP_ITEM_KNAME(crfsnp_item), (uint8_t)klen);
    CRFSNP_ITEM_KLEN(crfsnp_item) = (uint8_t)klen;
...
void crfsnp_item_print(LOG *log, const CRFSNP_ITEM *crfsnp_item)
{
    ...
    sys_print(log, "crfsnp_item %p: flag 0x%x [%s], stat %u, create time %u, hash %u, "
                   "key offset %u, parent %u, lru node (%u, %u), del node (%u, %u)\n",
                    ...
                    CRFSNPLRU_NODE_NEXT_POS(CRFSNP_ITEM_LRU_NODE(crfsnp_item)),
                    CRFSNPDEL_NODE_PREV_POS(CRFSNP_ITEM_DEL_NODE(crfsnp_item)),
                    CRFSNPDEL_NODE_NEXT_POS(CRFSNP_ITEM_DEL_NODE(crfsnp_item))
                    );
...
void crfsnp_item_and_key_print(LOG *log, const CRFSNP_ITEM *crfsnp_item)
{
    ...
    sys_log(log, "key: %.*s\n", CRFSNP_ITEM_KLEN(crfsnp_item), CRFSNP_ITEM_KNAME(crfsnp_item));
    if(CRFSNP_ITEM_FILE_IS_REG == CRFSNP_ITEM_DIR_FLAG(crfsnp_item))
    {
        CRFSNP_FNODE *crfsnp_fnode;
...
EC_BOOL crfsnp_item_is(const CRFSNP_ITEM *crfsnp_item, const uint32_t klen, const uint8_t *key)
{
    if(CRFSNP_KEY_MAX_SIZE < klen)/*overflow key*/
    {
        ...
        if(0 != memcmp((void *)md5_str, (void *)CRFSNP_ITEM_KNAME(crfsnp_item), md5_len))
        {
            return (EC_FALSE);
        }
        return (EC_TRUE);
    ...
    if(0 != strncmp((char *)key, (char *)CRFSNP_ITEM_KNAME(crfsnp_item), klen))
    {
        return (EC_FALSE);
    }
    return (EC_TRUE);
...
STATIC_CAST static CRFSNP_HEADER *__crfsnp_header_new(const uint32_t np_id, const UINT32 fsize, int fd, const uint8_t np_model)
{
    ...
    /*init key table*/
    crfsnpkey_pool_init(CRFSNP_HEADER_ITEMS_POOL(crfsnp_header), node_max_num, node_sizeof);
...
EC_BOOL crfsnp_free(CRFSNP *crfsnp)
{
    ...
    {
        crfsnp_clean(crfsnp);
        free_static_mem(MM_CRFSNP, crfsnp, LOC_CRFSNP_0019);
    }
    return (EC_TRUE);
}
    ...
    while(CRFSNPRB_ERR_POS != node_pos)
    {
        ...
        if(CRFSNP_ITEM_FILE_IS_REG == CRFSNP_ITEM_DIR_FLAG(crfsnp_item))
        {
            dbg_log(SEC_0081_CRFSNP, 0)(LOGSTDOUT, "error:crfsnp_insert_no_lock: np %u, find regular file at node_pos %u has same key: %.*s\n",
                                CRFSNP_ID(crfsnp), node_pos, CRFSNP_ITEM_KLEN(crfsnp_item), CRFSNP_ITEM_KNAME(crfsnp_item));
    ...
    && 0 == CRFSNP_ITEM_KLEN(crfsnp_item))
    {
        ...
        for(;;)
        {
            ...
            node_pos_t  = crfsnp_dnode_umount_son(crfsnp, parent_dnode, first_node_pos,
                                                  ...
                                                  CRFSNP_ITEM_KLEN(crfsnp_item_first), CRFSNP_ITEM_KNAME(crfsnp_item_first));
...
EC_BOOL crfsnp_path_name(const CRFSNP *crfsnp, const uint32_t node_pos, const uint32_t path_max_len, uint32_t *path_len, uint8_t *path)
{
    ...
    cstack = cstack_new(MM_IGNORE, LOC_CRFSNP_0020);
    ...
    cstack_free(cstack, LOC_CRFSNP_0021);
    return (EC_TRUE);
}
...
EC_BOOL crfsnp_path_name_cstr(const CRFSNP *crfsnp, const uint32_t node_pos, CSTRING *path_cstr)
{
    ...
    cstack = cstack_new(MM_IGNORE, LOC_CRFSNP_0022);
    ...
    while(EC_FALSE == cstack_is_empty(cstack))
    {
        ...
        if(CRFSNP_ITEM_FILE_IS_DIR == CRFSNP_ITEM_DIR_FLAG(crfsnp_item))
        {
            cstring_format(path_cstr, "%.*s/", CRFSNP_ITEM_KLEN(crfsnp_item), (char *)CRFSNP_ITEM_KNAME(crfsnp_item));
        }
        else if(CRFSNP_ITEM_FILE_IS_REG == CRFSNP_ITEM_DIR_FLAG(crfsnp_item))
        {
            cstring_format(path_cstr, "%.*s", CRFSNP_ITEM_KLEN(crfsnp_item), (char *)CRFSNP_ITEM_KNAME(crfsnp_item));
    ...
    cstack_clean(cstack, NULL_PTR);/*cleanup for safe reason*/
    cstack_free(cstack, LOC_CRFSNP_0023);
    return (EC_TRUE);
}
...
EC_BOOL crfsnp_seg_name_cstr(const CRFSNP *crfsnp, const uint32_t offset, CSTRING *seg_cstr)
{
    ...
    if(CRFSNP_ITEM_FILE_IS_DIR == CRFSNP_ITEM_DIR_FLAG(crfsnp_item))
    {
        cstring_format(seg_cstr, "%.*s/", CRFSNP_ITEM_KLEN(crfsnp_item), (char *)CRFSNP_ITEM_KNAME(crfsnp_item));
        return (EC_TRUE);
    }
    if(CRFSNP_ITEM_FILE_IS_REG == CRFSNP_ITEM_DIR_FLAG(crfsnp_item))
    {
...
STATIC_CAST static EC_BOOL __crfsnp_list_path_vec(const CRFSNP *crfsnp, const uint32_t node_pos, const uint8_t *prev_path_str, CVECTOR *path_cstr_vec)
{
    ...
    full_path_cstr = cstring_new(prev_path_str, LOC_CRFSNP_0024);
...
EC_BOOL crfsnp_list_path_vec(const CRFSNP *crfsnp, const uint32_t node_pos, CVECTOR *path_cstr_vec)
{
    ...
    path_cstr = cstring_new(NULL_PTR, LOC_CRFSNP_0025);
...
EC_BOOL crfsnp_list_seg_vec(const CRFSNP *crfsnp, const uint32_t node_pos, CVECTOR *seg_cstr_vec)
{
    ...
    if(CRFSNP_ITEM_FILE_IS_REG == CRFSNP_ITEM_DIR_FLAG(crfsnp_item))
    {
        ...
        seg_name_cstr = cstring_new(NULL_PTR, LOC_CRFSNP_0026);
...
CRFSNP *crfsnp_open(const char *np_root_dir, const uint32_t np_id)
{
    ...
    if(EC_FALSE == c_file_access(np_fname, F_OK))
    {
        ...
        safe_free(np_fname, LOC_CRFSNP_0027);
        return (NULL_PTR);
    }
    ...
    if(ERR_FD == fd)
    {
        ...
        safe_free(np_fname, LOC_CRFSNP_0028);
        return (NULL_PTR);
    }
    ...
    if(EC_FALSE == c_file_size(fd, &fsize))
    {
        ...
        safe_free(np_fname, LOC_CRFSNP_0029);
        c_file_close(fd);
        return (NULL_PTR);
    }
    ...
    {
        dbg_log(SEC_0081_CRFSNP, 0)(LOGSTDOUT, "error:crfsnp_open: open crfsnp file %s failed\n", np_fname);
        safe_free(np_fname, LOC_CRFSNP_0030);
        c_file_close(fd);
        return (NULL_PTR);
    }
    ...
    if(NULL_PTR == crfsnp)
    {
        ...
        safe_free(np_fname, LOC_CRFSNP_0031);
        c_file_close(fd);
...
EC_BOOL crfsnp_create_root_item(CRFSNP *crfsnp)
{
    ...
    //CRFSNP_ITEM_KNAME(crfsnp_item)[ 0 ] = '/';/*deprecated*/
    CRFSNP_ITEM_KNAME(crfsnp_item)[ 0 ] = key[ 0 ];
...
CRFSNP *crfsnp_clone(CRFSNP *src_crfsnp, const char *np_root_dir, const uint32_t des_np_id)
{
    ...
    if(EC_TRUE == c_file_access(des_np_fname, F_OK))/*exist*/
    {
        ...
        safe_free(des_np_fname, LOC_CRFSNP_0032);
        return (NULL_PTR);
    }
    ...
    if(ERR_FD == fd)
    {
        ...
        safe_free(des_np_fname, LOC_CRFSNP_0033);
        return (NULL_PTR);
    }
    ...
    if(EC_FALSE == c_file_truncate(fd, fsize))
    {
        ...
        safe_free(des_np_fname, LOC_CRFSNP_0034);
        c_file_close(fd);
        return (NULL_PTR);
    }
    ...
    if(NULL_PTR == des_crfsnp_header)
    {
        ...
        safe_free(des_np_fname, LOC_CRFSNP_0035);
        c_file_close(fd);
        return (NULL_PTR);
    }
    ...
    if(NULL_PTR == des_crfsnp)
    {
        ...
        safe_free(des_np_fname, LOC_CRFSNP_0036);
...
CRFSNP *crfsnp_create(const char *np_root_dir, const uint32_t np_id, const uint8_t np_model, const uint8_t hash_2nd_algo_id)
{
    ...
    ASSERT(256 == ((unsigned long)(&(((CRFSNP_HEADER *)0)->pool.rb_nodes))));
...
STATIC_CAST static EC_BOOL __crfsnp_get_item_full_path(const CRFSNP *crfsnp, const uint32_t node_pos, uint8_t **full_path, uint32_t *dflag)
{
    ...
    path = safe_malloc(CRFSNP_PATH_MAX_LEN, LOC_CRFSNP_0042);
    if(NULL_PTR == path)
    {
    ...
    while(EC_FALSE == cstack_is_empty(cstack))
    {
        ...
        if(CRFSNP_ITEM_FILE_IS_DIR == CRFSNP_ITEM_DIR_FLAG(crfsnp_item))
        {
            path_len += snprintf((char *)path + path_len, path_max_len - path_len, "%.*s/", CRFSNP_ITEM_KLEN(crfsnp_item), (char *)CRFSNP_ITEM_KNAME(crfsnp_item));
        }
        else if(CRFSNP_ITEM_FILE_IS_REG == CRFSNP_ITEM_DIR_FLAG(crfsnp_item))
        {
            path_len += snprintf((char *)path + path_len, path_max_len - path_len, "%.*s", CRFSNP_ITEM_KLEN(crfsnp_item), (char *)CRFSNP_ITEM_KNAME(crfsnp_item));
    ...
    cstack_free(cstack, LOC_CRFSNP_0045);
...
EC_BOOL crfsnp_show_item_full_path(LOG *log, const CRFSNP *crfsnp, const uint32_t node_pos)
{
    ...
    safe_free(path, LOC_CRFSNP_0046);
    return (EC_TRUE);
}
...
EC_BOOL crfsnp_mem_free(CRFSNP *crfsnp)
{
    if(NULL_PTR != crfsnp)
    {
        ...
        free_static_mem(MM_CRFSNP, crfsnp, LOC_CRFSNP_0047);
    }
    return (EC_TRUE);
}
