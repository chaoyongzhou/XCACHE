...
void api_ui_parse_cmdline(char* cmdline, int* argc_ptr, char*** argv_ptr) {
    ...
    cpy_cmdline = (char*)api_ui_malloc(sizeof(char) * i, LOC_API_0009);
    if(cpy_cmdline != NULL)
    {
    ...
    while (next_token(&token_instance) != NULL)
        (*argc_ptr)++;
    api_ui_free(cpy_cmdline, LOC_API_0010);

    (*argv_ptr) = (char**)api_ui_malloc(sizeof(char*) * (*argc_ptr), LOC_API_0011);
