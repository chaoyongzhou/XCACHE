...
STATIC_CAST static void kbuff_print_cache(LOG *log, const KBUFF *kbuff)
{
    ...
    dbg_log(SEC_0094_KBUFF, 5)(LOGSTDOUT, "kbuff %p: whole cache: \n", kbuff);
...
{
    dbg_log(SEC_0094_KBUFF, 5)(LOGSTDOUT, "kbuff %p: max_len = %ld, cur_len = %ld\n", kbuff, KBUFF_MAX_LEN(kbuff), KBUFF_CUR_LEN(kbuff));

    if( 0 == KBUFF_CUR_LEN(kbuff))
    {
        dbg_log(SEC_0094_KBUFF, 5)(LOGSTDOUT, "kbuff %p: cache: (null)\n"      , kbuff);
        return;
    }
...
EC_BOOL kbuff_write(KBUFF *kbuff, const UINT8 *in_buff, const UINT32 in_buff_max_len, UINT32 *in_buff_pos)
{
    ...
    {
        dbg_log(SEC_0094_KBUFF, 0)(LOGSTDOUT, "error:kbuff_write: kbuff %p is empty\n", kbuff);
        return (EC_FALSE);
    }
...
EC_BOOL kbuff_fread(KBUFF *kbuff, const UINT32 size, FILE *fp)
{
    ...
    {
        dbg_log(SEC_0094_KBUFF, 1)(LOGSTDOUT, "warn:kbuff_fread: kbuff %p with max_len = %ld, cur_len = %ld failed to resize to accept %ld bytes\n",
                           kbuff, KBUFF_MAX_LEN(kbuff), KBUFF_CUR_LEN(kbuff), size);
        return (EC_FALSE);
    }

    //dbg_log(SEC_0094_KBUFF, 3)(LOGSTDOUT, "info:kbuff_fread: kbuff %p, cur_len = %ld, max_len = %ld, size = %ld, fp = %lx\n", kbuff, KBUFF_CUR_LEN(kbuff), KBUFF_MAX_LEN(kbuff), size, fp);
...
EC_BOOL kbuff_fwrite(const KBUFF *kbuff, FILE *fp, UINT32 *pos)
{
    ...
    {
        dbg_log(SEC_0094_KBUFF, 1)(LOGSTDOUT, "warn:kbuff_fwrite: kbuff %p with %ld bytes cannot reach position %ld\n", kbuff, KBUFF_CUR_LEN(kbuff), (*pos));
        return (EC_FALSE);
    }

    //dbg_log(SEC_0094_KBUFF, 3)(LOGSTDOUT, "info:kbuff_fwrite: kbuff %p, cur_len = %ld, max_len = %ld, pos = %ld, fp = %lx\n", kbuff, KBUFF_CUR_LEN(kbuff), KBUFF_MAX_LEN(kbuff), (*pos), fp);
