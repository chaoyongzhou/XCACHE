...
EC_BOOL crfshttps_commit_http_get(CHTTP_NODE *chttp_node)
{
    ...
    else if (EC_TRUE == crfshttps_is_http_get_file_wait(chttp_node))
    {
        ret = crfshttps_commit_file_wait_get_request(chttp_node);
    }
    ...
    else if (EC_TRUE == crfshttps_is_http_get_wait_header(chttp_node))
    {
        ret = crfshttps_commit_wait_header_get_request(chttp_node);
    }
...
EC_BOOL crfshttps_is_http_get_file_wait(const CHTTP_NODE *chttp_node)
{
    const CBUFFER *uri_cbuffer;
...
EC_BOOL crfshttps_commit_file_wait_get_request(CHTTP_NODE *chttp_node)
{
    EC_BOOL ret;

    if(EC_FALSE == crfshttps_handle_file_wait_get_request(chttp_node))
    {
        dbg_log(SEC_0158_CRFSHTTPS, 0)(LOGSTDOUT, "error:crfshttps_commit_file_wait_get_request: handle 'GET' request failed\n");
        return (EC_FALSE);
    }

    if(EC_FALSE == crfshttps_make_file_wait_get_response(chttp_node))
    {
        dbg_log(SEC_0158_CRFSHTTPS, 0)(LOGSTDOUT, "error:crfshttps_commit_file_wait_get_request: make 'GET' response failed\n");
        return (EC_FALSE);
    }

    ret = crfshttps_commit_file_wait_get_response(chttp_node);
    if(EC_FALSE == ret)
    {
...
EC_BOOL crfshttps_handle_file_wait_get_request(CHTTP_NODE *chttp_node)
{
    CBUFFER       *uri_cbuffer;
    ...
    if(!(0 == req_body_chunk_num))
    {
        ...
        CHTTP_NODE_LOG_TIME_WHEN_DONE(chttp_node);
        CHTTP_NODE_LOG_STAT_WHEN_DONE(chttp_node, "RFS_ERR %s %u --", "GET", CHTTP_BAD_REQUEST);
        CHTTP_NODE_LOG_INFO_WHEN_DONE(chttp_node, "error: crfshttps_handle_file_wait_get_request: path %s", (char *)cstring_get_str(&path_cstr));
    ...
    if(NULL_PTR == v)
    {
        CHTTP_NODE_LOG_TIME_WHEN_DONE(chttp_node);
        CHTTP_NODE_LOG_STAT_WHEN_DONE(chttp_node, "RFS_ERR %s %u --", "GET", CHTTP_BAD_REQUEST);
        CHTTP_NODE_LOG_INFO_WHEN_DONE(chttp_node, "error: crfshttps_handle_file_wait_get_request: path %s, tcid absence", (char *)cstring_get_str(&path_cstr));
    ...
    if(NULL_PTR != wait_data_str && EC_FALSE == c_str_to_bool(wait_data_str))
    {
        ...
        if(EC_FALSE == crfs_file_wait(CSOCKET_CNODE_MODI(csocket_cnode), &mod_node, &path_cstr, NULL_PTR, &data_ready))
        {
            ...
            CHTTP_NODE_LOG_TIME_WHEN_DONE(chttp_node);
            CHTTP_NODE_LOG_STAT_WHEN_DONE(chttp_node, "RFS_FAIL %s %u --", "GET", CHTTP_INTERNAL_SERVER_ERROR);
            CHTTP_NODE_LOG_INFO_WHEN_DONE(chttp_node, "error:crfshttps_handle_file_wait_get_request: crfs wait %s only failed", (char *)cstring_get_str(&path_cstr));
        ...
        CHTTP_NODE_LOG_INFO_WHEN_DONE(chttp_node, "[DEBUG] crfshttps_handle_file_wait_get_request: crfs wait %s only done", (char *)cstring_get_str(&path_cstr));
    ...
    if(NULL_PTR != store_offset_str)
    {
        ...
        if(EC_FALSE == crfs_file_wait_e(CSOCKET_CNODE_MODI(csocket_cnode), &mod_node, &path_cstr, &offset, max_len, content_cbytes, &data_ready))
        {
            ...
            CHTTP_NODE_LOG_TIME_WHEN_DONE(chttp_node);
            CHTTP_NODE_LOG_STAT_WHEN_DONE(chttp_node, "RFS_FAIL %s %u --", "GET", CHTTP_NOT_FOUND);
            CHTTP_NODE_LOG_INFO_WHEN_DONE(chttp_node, "error:crfshttps_handle_file_wait_get_request: crfs wait %s with offset %u, size %u failed", (char *)cstring_get_str(&path_cstr), store_offset, store_size);
        ...
        CHTTP_NODE_LOG_TIME_WHEN_DONE(chttp_node);
        CHTTP_NODE_LOG_STAT_WHEN_DONE(chttp_node, "RFS_SUCC %s %u %ld", "GET", CHTTP_OK, CBYTES_LEN(content_cbytes));
        CHTTP_NODE_LOG_INFO_WHEN_DONE(chttp_node, "[DEBUG] crfshttps_handle_file_wait_get_request: crfs wait %s with offset %u, size %u done", (char *)cstring_get_str(&path_cstr), store_offset, store_size);
    ...
    else/*wait whole file content*/
    {
        ...
        if(EC_FALSE == crfs_file_wait(CSOCKET_CNODE_MODI(csocket_cnode), &mod_node, &path_cstr, content_cbytes, &data_ready))
        {
            ...
            CHTTP_NODE_LOG_TIME_WHEN_DONE(chttp_node);
            CHTTP_NODE_LOG_STAT_WHEN_DONE(chttp_node, "RFS_FAIL %s %u --", "GET", CHTTP_NOT_FOUND);
            CHTTP_NODE_LOG_INFO_WHEN_DONE(chttp_node, "error:crfshttps_handle_file_wait_get_request: crfs wait %s failed", (char *)cstring_get_str(&path_cstr));
        ...
        CHTTP_NODE_LOG_TIME_WHEN_DONE(chttp_node);
        CHTTP_NODE_LOG_STAT_WHEN_DONE(chttp_node, "RFS_SUCC %s %u %ld", "GET", CHTTP_OK, CBYTES_LEN(content_cbytes));
        CHTTP_NODE_LOG_INFO_WHEN_DONE(chttp_node, "[DEBUG] crfshttps_handle_file_wait_get_request: crfs wait %s done", (char *)cstring_get_str(&path_cstr));
...
EC_BOOL crfshttps_make_file_wait_get_response(CHTTP_NODE *chttp_node)
{
    CBYTES        *content_cbytes;
    uint64_t       content_len;
...
EC_BOOL crfshttps_commit_file_wait_get_response(CHTTP_NODE *chttp_node)
{
    CSOCKET_CNODE * csocket_cnode;
    ...
    if(NULL_PTR == csocket_cnode)
    {
        dbg_log(SEC_0158_CRFSHTTPS, 0)(LOGSTDOUT, "error:crfshttps_commit_file_wait_get_response: csocket_cnode of chttp_node %p is null\n", chttp_node);
        return (EC_FALSE);
    }
...
EC_BOOL crfshttps_is_http_get_wait_header(const CHTTP_NODE *chttp_node)
{
    const CBUFFER *uri_cbuffer;
...
EC_BOOL crfshttps_commit_wait_header_get_request(CHTTP_NODE *chttp_node)
{
    EC_BOOL ret;

    if(EC_FALSE == crfshttps_handle_wait_header_get_request(chttp_node))
    {
        dbg_log(SEC_0158_CRFSHTTPS, 0)(LOGSTDOUT, "error:crfshttps_commit_wait_header_get_request: handle 'GET' request failed\n");
        return (EC_FALSE);
    }

    if(EC_FALSE == crfshttps_make_wait_header_get_response(chttp_node))
    {
        dbg_log(SEC_0158_CRFSHTTPS, 0)(LOGSTDOUT, "error:crfshttps_commit_wait_header_get_request: make 'GET' response failed\n");
        return (EC_FALSE);
    }

    ret = crfshttps_commit_wait_header_get_response(chttp_node);
    if(EC_FALSE == ret)
    {
...
EC_BOOL crfshttps_handle_wait_header_get_request(CHTTP_NODE *chttp_node)
{
    CBUFFER       *uri_cbuffer;
    ...
    if(!(0 == req_body_chunk_num))
    {
        ...
        CHTTP_NODE_LOG_TIME_WHEN_DONE(chttp_node);
        CHTTP_NODE_LOG_STAT_WHEN_DONE(chttp_node, "RFS_ERR %s %u --", "GET", CHTTP_BAD_REQUEST);
        CHTTP_NODE_LOG_INFO_WHEN_DONE(chttp_node, "error: crfshttps_handle_wait_header_get_request: path %s", (char *)cstring_get_str(&path_cstr));
    ...
    if(NULL_PTR == v)
    {
        CHTTP_NODE_LOG_TIME_WHEN_DONE(chttp_node);
        CHTTP_NODE_LOG_STAT_WHEN_DONE(chttp_node, "RFS_ERR %s %u --", "GET", CHTTP_BAD_REQUEST);
        CHTTP_NODE_LOG_INFO_WHEN_DONE(chttp_node, "error: crfshttps_handle_wait_header_get_request: path %s, tcid absence", (char *)cstring_get_str(&path_cstr));
    ...
    if(NULL_PTR == wait_num)
    {
        ...
        if(NULL_PTR == wait_key)
        {
            ...
            CHTTP_NODE_LOG_TIME_WHEN_DONE(chttp_node);
            CHTTP_NODE_LOG_STAT_WHEN_DONE(chttp_node, "RFS_FAIL %s %u --", "GET", CHTTP_FORBIDDEN);
            CHTTP_NODE_LOG_INFO_WHEN_DONE(chttp_node, "error:crfshttps_handle_wait_header_get_request: crfs wait %s failed due to 'wait-key' absence", (char *)cstring_get_str(&path_cstr));
        ...
        if(NULL_PTR == wait_val)
        {
            ...
            CHTTP_NODE_LOG_TIME_WHEN_DONE(chttp_node);
            CHTTP_NODE_LOG_STAT_WHEN_DONE(chttp_node, "RFS_FAIL %s %u --", "GET", CHTTP_FORBIDDEN);
            CHTTP_NODE_LOG_INFO_WHEN_DONE(chttp_node, "error:crfshttps_handle_wait_header_get_request: crfs wait %s failed due to 'wait-val' absence", (char *)cstring_get_str(&path_cstr));
        ...
        if(EC_FALSE == crfs_wait_http_header(CSOCKET_CNODE_MODI(csocket_cnode), &mod_node, &path_cstr, &wait_key_cstr, &wait_val_cstr, &header_ready))
        {
            ...
            CHTTP_NODE_LOG_TIME_WHEN_DONE(chttp_node);
            CHTTP_NODE_LOG_STAT_WHEN_DONE(chttp_node, "RFS_FAIL %s %u --", "GET", CHTTP_FORBIDDEN);
            CHTTP_NODE_LOG_INFO_WHEN_DONE(chttp_node, "error:crfshttps_handle_wait_header_get_request: crfs wait %s failed", (char *)cstring_get_str(&path_cstr));
        ...
        CHTTP_NODE_LOG_TIME_WHEN_DONE(chttp_node);
        CHTTP_NODE_LOG_STAT_WHEN_DONE(chttp_node, "RFS_SUCC %s %u --", "GET", CHTTP_OK);
        CHTTP_NODE_LOG_INFO_WHEN_DONE(chttp_node, "[DEBUG] crfshttps_handle_wait_header_get_request: crfs wait %s done", (char *)cstring_get_str(&path_cstr));
    ...
    if(NULL_PTR == cstrkv_mgr)
    {
        ...
        CHTTP_NODE_LOG_TIME_WHEN_DONE(chttp_node);
        CHTTP_NODE_LOG_STAT_WHEN_DONE(chttp_node, "RFS_FAIL %s %u --", "GET", CHTTP_INTERNAL_SERVER_ERROR);
        CHTTP_NODE_LOG_INFO_WHEN_DONE(chttp_node, "error:crfshttps_handle_wait_header_get_request: crfs wait %s failed due to new cstrkv_mgr failed",
                (char *)cstring_get_str(&path_cstr));
    ...
    for(idx = 0; idx < num; idx ++)
    {
        ...
        if(NULL_PTR == wait_key)
        {
            ...
            CHTTP_NODE_LOG_TIME_WHEN_DONE(chttp_node);
            CHTTP_NODE_LOG_STAT_WHEN_DONE(chttp_node, "RFS_FAIL %s %u --", "GET", CHTTP_FORBIDDEN);
            CHTTP_NODE_LOG_INFO_WHEN_DONE(chttp_node, "error:crfshttps_handle_wait_header_get_request: crfs wait %s failed due to '%s' absence",
                    (char *)cstring_get_str(&path_cstr), (char *)wait_key_tag);
        ...
        if(EC_FALSE == cstrkv_mgr_add_kv_str(cstrkv_mgr, wait_key, wait_val))
        {
            ...
            CHTTP_NODE_LOG_TIME_WHEN_DONE(chttp_node);
            CHTTP_NODE_LOG_STAT_WHEN_DONE(chttp_node, "RFS_FAIL %s %u --", "GET", CHTTP_INTERNAL_SERVER_ERROR);
            CHTTP_NODE_LOG_INFO_WHEN_DONE(chttp_node, "error:crfshttps_handle_wait_header_get_request: crfs wait %s failed due to add '%s:%s' failed",
                    (char *)cstring_get_str(&path_cstr), wait_key, wait_val);
...
EC_BOOL crfshttps_make_wait_header_get_response(CHTTP_NODE *chttp_node)
{
    if(do_log(SEC_0158_CRFSHTTPS, 9))
    {
        CBUFFER       *uri_cbuffer;
...
EC_BOOL crfshttps_commit_wait_header_get_response(CHTTP_NODE *chttp_node)
{
    CSOCKET_CNODE * csocket_cnode;
    ...
    if(NULL_PTR == csocket_cnode)
    {
        dbg_log(SEC_0158_CRFSHTTPS, 0)(LOGSTDOUT, "error:crfshttps_commit_wait_header_get_response: csocket_cnode of chttp_node %p is null\n", chttp_node);
        return (EC_FALSE);
    }
