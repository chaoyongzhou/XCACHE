...
EC_BOOL crfsmc_clean(CRFSMC *crfsmc)
{
    ...
    if(NULL_PTR != CRFSMC_MCACHE(crfsmc))
    {
        safe_free(CRFSMC_MCACHE(crfsmc), LOC_CRFSMC_0004);
...
EC_BOOL crfsmc_free(CRFSMC *crfsmc)
{
    ...
    {
        crfsmc_clean(crfsmc);
        safe_free(crfsmc, LOC_CRFSMC_0005);
    }
...
EC_BOOL crfsmc_read_e_dn_no_lock(CRFSMC *crfsmc, const CRFSNP_FNODE *crfsnp_fnode, UINT32 *store_offset, const UINT32 store_size, CBYTES *cbytes)
{
    ...
    {
        dbg_log(SEC_0140_CRFSMC, 0)(LOGSTDOUT, "error:crfsmc_read_e_dn_no_lock: read file failed due to offset %ld overflow file size %u\n",
    ...
    if(CBYTES_LEN(cbytes) < max_len)
    {
        ...
        {
            SAFE_FREE(CBYTES_BUF(cbytes), LOC_CRFSMC_0006);
        }
        CBYTES_BUF(cbytes) = (UINT8 *)SAFE_MALLOC(max_len, LOC_CRFSMC_0007);
