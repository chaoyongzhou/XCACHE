...
EC_BOOL task_brd_task_mgr_add(TASK_BRD *task_brd, TASK_MGR *task_mgr)
{
    ...
    switch(TASK_MGR_PRIO(task_mgr))
    {
        ...
        case TASK_PRIO_HIGH:
        {
            ...
            CLIST_LOCK(task_mgr_list, LOC_TASK_0140);
            CLIST_LOOP_NEXT(task_mgr_list, clist_data)
            {
            ...
            CLIST_UNLOCK(task_mgr_list, LOC_TASK_0142);
...
EC_BOOL task_brd_task_mgr_match(const TASK_BRD *task_brd, const TASK_RSP *task_rsp, TASK_MGR **task_mgr_ret)
{
    ...
    CLIST_LOCK(task_mgr_list, LOC_TASK_0156);
    CLIST_LOOP_NEXT(task_mgr_list, clist_data)
    {
        TASK_MGR *task_mgr;
    ...
    CLIST_UNLOCK(task_mgr_list, LOC_TASK_0158);
    return (EC_FALSE);
}
...
EC_BOOL task_mgr_recv(TASK_MGR *task_mgr)
{
    ...
    TASK_MGR_CRWLOCK_RDLOCK(task_mgr, LOC_TASK_0189);
    ...
    TASK_MGR_CRWLOCK_UNLOCK(task_mgr, LOC_TASK_0190);
...
EC_BOOL task_mgr_reschedule_to(TASK_BRD *task_brd, TASK_MGR *task_mgr, const UINT32 tcid, const UINT32 comm)
{
    ...
    CLIST_LOOP_NEXT(task_queue, clist_data)
    {
        ...
        if(EC_TRUE == TASK_REQ_RECV_MOD_FLAG(task_req))
        {
            ...
            TASK_MGR_COUNTER_DEC_BY_TASK_REQ(task_mgr, TASK_MGR_COUNTER_TASK_REQ_IS_SENT, TASK_NODE_REQ(task_node), LOC_TASK_0192);
    ...
    CLIST_UNLOCK(task_queue, LOC_TASK_0194);
    return (EC_TRUE);
}
...
EC_BOOL task_mgr_discard_to(TASK_BRD *task_brd, TASK_MGR *task_mgr, const UINT32 tcid, const UINT32 comm)
{
    ...
    CLIST_LOOP_NEXT(task_queue, clist_data)
    {
        ...
        if(TASK_REQ_SENDAGN == TASK_NODE_STATUS(task_node))/*Feb 11, 2018*/
        {
            ...
            TASK_MGR_COUNTER_INC_BY_TASK_REQ(task_mgr, TASK_MGR_COUNTER_TASK_REQ_DISCARD, TASK_NODE_REQ(task_node), LOC_TASK_0197);
        ...
        TASK_NODE_CMUTEX_LOCK(task_node, LOC_TASK_0198);
        if(TASK_REQ_IS_SENT == TASK_NODE_STATUS(task_node))
        {
            TASK_REQ *task_req;

            TASK_MGR_COUNTER_DEC_BY_TASK_REQ(task_mgr, TASK_MGR_COUNTER_TASK_REQ_IS_SENT, TASK_NODE_REQ(task_node), LOC_TASK_0199);
            TASK_MGR_COUNTER_INC_BY_TASK_REQ(task_mgr, TASK_MGR_COUNTER_TASK_REQ_DISCARD, TASK_NODE_REQ(task_node), LOC_TASK_0200);
        ...
        TASK_NODE_CMUTEX_UNLOCK(task_node, LOC_TASK_0202);
    }
...
EC_BOOL task_mgr_print(LOG *log, TASK_MGR *task_mgr)
{
    ...
    CLIST_LOCK(task_queue, LOC_TASK_0204);
    CLIST_LOOP_NEXT(task_queue, clist_data)
    {
        TASK_NODE  *task_node;
        TASK_REQ   *task_req;
...
void task_brd_recv_task_mgr_list(TASK_BRD *task_brd)
{
    ...
    CLIST_LOOP_NEXT(task_mgr_list, clist_data)
    {
        ...
        if(EC_TRUE == task_mgr_recv(task_mgr))
        {
            ...
            {
                TASK_MGR_CCOND_RELEASE(task_mgr, LOC_TASK_0209);
            }
            else
            {
    ...
    CLIST_UNLOCK(task_mgr_list, LOC_TASK_0210);
    return;
}
...
void task_brd_aging_task_mgr_list(TASK_BRD *task_brd)
{
    ...
    CLIST_LOCK(task_mgr_list, LOC_TASK_0211);
    CLIST_LOOP_NEXT(task_mgr_list, clist_data)
    {
        TASK_MGR *task_mgr;
...
EC_BOOL task_brd_cload_stat_update(TASK_BRD *task_brd)
{
    ...
    for(;;)
    {
        ...
        {
            //sched_yield();
            c_sleep(1, LOC_TASK_0214);/*fuck, maybe can use timer to trigger*/
            continue;
        }
...
{
    TASK_BRD_SEQNO_CMUTEX_INT(task_brd, LOC_TASK_0215);

    CLIST_INIT_LOCK(CTHREAD_POOL_WORKER_IDLE_LIST(TASK_REQ_CTHREAD_POOL(task_brd)), LOC_TASK_0216);
    CLIST_INIT_LOCK(CTHREAD_POOL_WORKER_BUSY_LIST(TASK_REQ_CTHREAD_POOL(task_brd)), LOC_TASK_0217);

#if (SWITCH_ON == CROUTINE_SUPPORT_CTHREAD_SWITCH)
    cmutex_init(CTHREAD_POOL_WORKER_CMUTEX(TASK_REQ_CTHREAD_POOL(task_brd)), CMUTEX_PROCESS_PRIVATE, LOC_TASK_0218);
#endif/*(SWITCH_ON == CROUTINE_SUPPORT_CTHREAD_SWITCH)*/

    CVECTOR_INIT_LOCK(TASK_BRD_MD_NODE_TBL(task_brd), LOC_TASK_0219);

    CLIST_INIT_LOCK(TASK_BRD_CLOAD_MGR(task_brd), LOC_TASK_0220);
    CVECTOR_INIT_LOCK(TASK_BRD_BROKEN_TBL(task_brd), LOC_TASK_0221);
    CLIST_INIT_LOCK(TASK_BRD_MOD_MGR_LIST(task_brd), LOC_TASK_0222);

    CLIST_INIT_LOCK(TASK_BRD_RECV_TASK_MGR_LIST(task_brd), LOC_TASK_0223);

    CLIST_INIT_LOCK(TASK_BRD_CONTEXT_LIST(task_brd), LOC_TASK_0224);
    CLIST_INIT_LOCK(TASK_BRD_REPORT_LIST(task_brd), LOC_TASK_0225);

    CLIST_INIT_LOCK(TASK_BRD_QUEUE(task_brd, TASK_RECVING_QUEUE), LOC_TASK_0226);
    CLIST_INIT_LOCK(TASK_BRD_QUEUE(task_brd, TASK_IS_RECV_QUEUE), LOC_TASK_0227);
    CLIST_INIT_LOCK(TASK_BRD_QUEUE(task_brd, TASK_TO_SEND_QUEUE), LOC_TASK_0228);
    CLIST_INIT_LOCK(TASK_BRD_QUEUE(task_brd, TASK_SENDING_QUEUE), LOC_TASK_0229);

    CVECTOR_INIT_LOCK(TASK_BRD_RANK_TBL(task_brd), LOC_TASK_0230);
    CLIST_INIT_LOCK(TASK_BRD_CBTIMER_LIST(task_brd), LOC_TASK_0231);
...
{
    if(EC_FALSE == creg_static_mem_tbl_add(mm_type, mm_name, block_num, type_size, LOC_TASK_0232))
    {
