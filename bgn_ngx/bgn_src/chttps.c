...
STATIC_CAST static EC_BOOL __chttps_on_recv_complete(CHTTPS_NODE *chttps_node)
{
...
/*private interface, not for http parser*/
STATIC_CAST static EC_BOOL __chttps_on_send_complete(CHTTPS_NODE *chttps_node)
{
    CSOCKET_CNODE *csocket_cnode;
...
CHTTPS_NODE *chttps_node_new(const UINT32 type)
{
    ...
    alloc_static_mem(MM_CHTTPS_NODE, &chttps_node, LOC_CHTTPS_0006);
    if(NULL_PTR == chttps_node)
    {
...
EC_BOOL chttps_node_clean(CHTTPS_NODE *chttps_node)
{
    if(NULL_PTR != chttps_node)
    {
        ...
        {
            croutine_cond_free(CHTTPS_NODE_CROUTINE_COND(chttps_node), LOC_CHTTPS_0008);
            CHTTPS_NODE_CROUTINE_COND(chttps_node) = NULL_PTR;
        }
...
EC_BOOL chttps_node_free(CHTTPS_NODE *chttps_node)
{
    if(NULL_PTR != chttps_node)
    {
        ...
        free_static_mem(MM_CHTTPS_NODE, chttps_node, LOC_CHTTPS_0009);
    }
...
EC_BOOL chttps_srv_accept_once(CSRV *csrv, EC_BOOL *continue_flag)
{
    ...
    if(EC_TRUE == ret)
    {
        ...
        csocket_cnode = csocket_cnode_new(LOC_CHTTPS_0011);/*here do not know the remote client srv port*/
        if(NULL_PTR == csocket_cnode)
        {
...
EC_BOOL chttps_node_complete(CHTTPS_NODE *chttps_node, CSOCKET_CNODE *csocket_cnode)
{
    ...
    dbg_log(SEC_0157_CHTTPS, 9)(LOGSTDOUT, "[DEBUG] chttps_node_complete: socket %d, enter\n", sockfd);
...
EC_BOOL chttps_node_timeout(CHTTPS_NODE *chttps_node, CSOCKET_CNODE *csocket_cnode)
{
    ...
    if(CHTTP_TYPE_DO_CLT_CHK == CHTTPS_NODE_TYPE(chttps_node))/*client side*/
    {
        ...
        {
            CHTTPS_NODE_COROUTINE_RESTORE(chttps_node) = BIT_TRUE;
            croutine_cond_release(CHTTPS_NODE_CROUTINE_COND(chttps_node), LOC_CHTTPS_0017);
        }
...
EC_BOOL chttps_node_shutdown(CHTTPS_NODE *chttps_node, CSOCKET_CNODE *csocket_cnode)
{
    ...
    if(CHTTP_TYPE_DO_CLT_RSP == CHTTPS_NODE_TYPE(chttps_node)) /*client side*/
    {
        ...
        {
            CHTTPS_NODE_COROUTINE_RESTORE(chttps_node) = BIT_TRUE;
            croutine_cond_release(CHTTPS_NODE_CROUTINE_COND(chttps_node), LOC_CHTTPS_0018);
        }
    ...
    if(CHTTP_TYPE_DO_CLT_CHK == CHTTPS_NODE_TYPE(chttps_node))/*client side*/
    {
        ...
        {
            CHTTPS_NODE_COROUTINE_RESTORE(chttps_node) = BIT_TRUE;
            croutine_cond_release(CHTTPS_NODE_CROUTINE_COND(chttps_node), LOC_CHTTPS_0019);
        }
...
CHTTPS_REST *chttps_rest_new()
{
    ...
    alloc_static_mem(MM_CHTTPS_REST, &chttps_rest, LOC_CHTTPS_0020);
...
EC_BOOL chttps_rest_free(CHTTPS_REST *chttps_rest)
{
    if(NULL_PTR != chttps_rest)
    {
        ...
        free_static_mem(MM_CHTTPS_REST, chttps_rest, LOC_CHTTPS_0021);
    }
...
EC_BOOL chttps_rest_list_push(const char *name, EC_BOOL (*commit)(CHTTPS_NODE *))
{
    ...
    {
        g_chttps_rest_list = clist_new(MM_CHTTPS_REST, LOC_CHTTPS_0022);
        if(NULL_PTR == g_chttps_rest_list)
        {
...
EC_BOOL chttps_defer_request_queue_init()
{
    ...
    {
        cqueue_init(&g_chttps_defer_request_queue, MM_CHTTPS_NODE, LOC_CHTTPS_0023);
...
EC_BOOL chttps_node_connect(CHTTPS_NODE *chttps_node, const UINT32 ipaddr, const UINT32 port)
{
    ...
    else
    {
        ...
        csocket_cnode = csocket_cnode_new(LOC_CHTTPS_0024);
        if(NULL_PTR == csocket_cnode)
        {
...
EC_BOOL chttps_node_recv_rsp(CHTTPS_NODE *chttps_node, CSOCKET_CNODE *csocket_cnode)
{
    ...
    if(BIT_TRUE == CHTTPS_NODE_RECV_COMPLETE(chttps_node))
    {
...
EC_BOOL chttps_request_basic(const CHTTP_REQ *chttp_req, CHTTP_RSP *chttp_rsp, CHTTP_STAT *chttp_stat)
{
    ...
    croutine_cond = croutine_cond_new(0/*never timeout*/, LOC_CHTTPS_0025);
    ...
    croutine_cond_reserve(croutine_cond, 1, LOC_CHTTPS_0026);
    croutine_cond_wait(croutine_cond, LOC_CHTTPS_0027);
