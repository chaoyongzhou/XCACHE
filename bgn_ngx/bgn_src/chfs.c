...
*
**/
static EC_BOOL __chfs_reserve_hash_dn(const UINT32 chfs_md_id, const UINT32 data_len, const CSTRING *file_path, CHFSNP_FNODE *chfsnp_fnode)
{
    CHFS_MD      *chfs_md;
...
static EC_BOOL __chfs_wait_file_owner_cmp(const MOD_NODE *mod_node, const UINT32 tcid)
{
...
static EC_BOOL __chfs_file_wait(const UINT32 chfs_md_id, const UINT32 tcid, const CSTRING *file_path)
{
    CHFS_MD          *chfs_md;
...
static EC_BOOL __chfs_locked_file_need_retire(const CHFS_LOCKED_FILE *chfs_locked_file)
{
    CTIMET cur_time;
    REAL diff_nsec;
...
static EC_BOOL __chfs_locked_file_retire(CRB_TREE *crbtree, const CRB_NODE *node)
{
    CHFS_LOCKED_FILE *chfs_locked_file;
...
static EC_BOOL __chfs_file_lock(const UINT32 chfs_md_id, const UINT32 tcid, const CSTRING *file_path, const UINT32 expire_nsec, CBYTES *token, UINT32 *locked_already)
{
    CHFS_MD          *chfs_md;
...
static EC_BOOL __chfs_file_unlock(const UINT32 chfs_md_id, const CSTRING *file_path, const CBYTES *token)
{
    CHFS_MD          *chfs_md;
...
EC_BOOL chfs_add_disk(const UINT32 chfs_md_id, const UINT32 disk_no)
{
    ...
    CHFS_WRLOCK(chfs_md, LOC_CHFS_0050);
    if(EC_FALSE == crfsdn_add_disk(CHFS_MD_DN(chfs_md), (uint16_t)disk_no))
    {
        CHFS_UNLOCK(chfs_md, LOC_CHFS_0051);
...
EC_BOOL chfs_del_disk(const UINT32 chfs_md_id, const UINT32 disk_no)
{
    ...
    CHFS_WRLOCK(chfs_md, LOC_CHFS_0053);
    if(EC_FALSE == crfsdn_del_disk(CHFS_MD_DN(chfs_md), (uint16_t)disk_no))
    {
        CHFS_UNLOCK(chfs_md, LOC_CHFS_0054);
...
EC_BOOL chfs_mount_disk(const UINT32 chfs_md_id, const UINT32 disk_no)
{
    ...
    CHFS_WRLOCK(chfs_md, LOC_CHFS_0056);
    if(EC_FALSE == crfsdn_mount_disk(CHFS_MD_DN(chfs_md), (uint16_t)disk_no))
    {
        CHFS_UNLOCK(chfs_md, LOC_CHFS_0057);
...
EC_BOOL chfs_umount_disk(const UINT32 chfs_md_id, const UINT32 disk_no)
{
    ...
    CHFS_WRLOCK(chfs_md, LOC_CHFS_0059);
    if(EC_FALSE == crfsdn_umount_disk(CHFS_MD_DN(chfs_md), (uint16_t)disk_no))
    {
        CHFS_UNLOCK(chfs_md, LOC_CHFS_0060);
...
EC_BOOL chfs_read_dn(const UINT32 chfs_md_id, const CHFSNP_FNODE *chfsnp_fnode, CBYTES *cbytes)
{
    ...
    if(CBYTES_LEN(cbytes) < file_size)
    {
        ...
        {
            SAFE_FREE(CBYTES_BUF(cbytes), LOC_CHFS_0065);
        }
        CBYTES_BUF(cbytes) = (UINT8 *)SAFE_MALLOC(file_size, LOC_CHFS_0066);
        CBYTES_LEN(cbytes) = 0;
    }

    crfsdn_rdlock(CHFS_MD_DN(chfs_md), LOC_CHFS_0067);
    if(EC_FALSE == crfsdn_read_p(CHFS_MD_DN(chfs_md), disk_no, block_no, page_no, file_size, CBYTES_BUF(cbytes), &(CBYTES_LEN(cbytes))))
    {
        crfsdn_unlock(CHFS_MD_DN(chfs_md), LOC_CHFS_0068);
...
EC_BOOL chfs_read_e_dn(const UINT32 chfs_md_id, const CHFSNP_FNODE *chfsnp_fnode, UINT32 *offset, const UINT32 max_len, CBYTES *cbytes)
{
    ...
    if(CBYTES_LEN(cbytes) < file_size)
    {
        if(NULL_PTR != CBYTES_BUF(cbytes))
        {
            SAFE_FREE(CBYTES_BUF(cbytes), LOC_CHFS_0070);
        }
        CBYTES_BUF(cbytes) = (UINT8 *)SAFE_MALLOC(max_len_t, LOC_CHFS_0071);
        CBYTES_LEN(cbytes) = 0;
    }

    crfsdn_rdlock(CHFS_MD_DN(chfs_md), LOC_CHFS_0072);
...
EC_BOOL chfs_write_npp(const UINT32 chfs_md_id, const CSTRING *file_path, const CHFSNP_FNODE *chfsnp_fnode)
{
    ...
    chfsnp_mgr_wrlock(CHFS_MD_NPP(chfs_md), LOC_CHFS_0075);
...
EC_BOOL chfs_read_npp(const UINT32 chfs_md_id, const CSTRING *file_path, CHFSNP_FNODE *chfsnp_fnode)
{
    ...
    chfsnp_mgr_rdlock(CHFS_MD_NPP(chfs_md), LOC_CHFS_0078);
...
EC_BOOL chfs_delete_npp(const UINT32 chfs_md_id, const CSTRING *path)
{
    ...
    chfsnp_mgr_wrlock(CHFS_MD_NPP(chfs_md), LOC_CHFS_0081);
...
static EC_BOOL __chfs_delete_dn(const UINT32 chfs_md_id, const CHFSNP_FNODE *chfsnp_fnode)
{
    ...
    crfsdn_wrlock(CHFS_MD_DN(chfs_md), LOC_CHFS_0084);
    if(EC_FALSE == crfsdn_remove(CHFS_MD_DN(chfs_md), disk_no, block_no, page_no, file_size))
    {
        crfsdn_unlock(CHFS_MD_DN(chfs_md), LOC_CHFS_0085);
...
EC_BOOL chfs_delete_dn(const UINT32 chfs_md_id, const CHFSNP_FNODE *chfsnp_fnode)
{
    ...
    crfsdn_wrlock(CHFS_MD_DN(chfs_md), LOC_CHFS_0087);
    if(EC_FALSE == crfsdn_remove(CHFS_MD_DN(chfs_md), disk_no, block_no, page_no, file_size))
    {
        crfsdn_unlock(CHFS_MD_DN(chfs_md), LOC_CHFS_0088); 
...
EC_BOOL chfs_qfile(const UINT32 chfs_md_id, const CSTRING *file_path, CHFSNP_ITEM  *chfsnp_item)
{
    ...
    chfsnp_mgr_rdlock(CHFS_MD_NPP(chfs_md), LOC_CHFS_0090);
    ...
    {
        chfsnp_mgr_unlock(CHFS_MD_NPP(chfs_md), LOC_CHFS_0091);
...
EC_BOOL chfs_flush_npp(const UINT32 chfs_md_id)
{
    ...
    chfsnp_mgr_wrlock(CHFS_MD_NPP(chfs_md), LOC_CHFS_0093);
    if(EC_FALSE == chfsnp_mgr_flush(CHFS_MD_NPP(chfs_md)))
    {
        chfsnp_mgr_unlock(CHFS_MD_NPP(chfs_md), LOC_CHFS_0094);
...
EC_BOOL chfs_flush_dn(const UINT32 chfs_md_id)
{
    ...
    crfsdn_wrlock(CHFS_MD_DN(chfs_md), LOC_CHFS_0096);
    if(EC_FALSE == crfsdn_flush(CHFS_MD_DN(chfs_md)))
    {
        crfsdn_unlock(CHFS_MD_DN(chfs_md), LOC_CHFS_0097);
...
EC_BOOL chfs_file_num(const UINT32 chfs_md_id, UINT32 *file_num)
{
    ...
    chfsnp_mgr_wrlock(CHFS_MD_NPP(chfs_md), LOC_CHFS_0099);
...
EC_BOOL chfs_file_size(const UINT32 chfs_md_id, const CSTRING *path_cstr, UINT32 *file_size)
{
    ...
    chfsnp_mgr_wrlock(CHFS_MD_NPP(chfs_md), LOC_CHFS_0102);
    if(EC_FALSE == chfsnp_mgr_file_size(CHFS_MD_NPP(chfs_md), path_cstr, file_size))
    {
        chfsnp_mgr_unlock(CHFS_MD_NPP(chfs_md), LOC_CHFS_0103);
...
EC_BOOL chfs_show_npp(const UINT32 chfs_md_id, LOG *log)
{
    ...
    chfsnp_mgr_rdlock(CHFS_MD_NPP(chfs_md), LOC_CHFS_0108);
 
    chfsnp_mgr_print(log, CHFS_MD_NPP(chfs_md));
 
    chfsnp_mgr_unlock(CHFS_MD_NPP(chfs_md), LOC_CHFS_0109);
...
EC_BOOL chfs_show_dn(const UINT32 chfs_md_id, LOG *log)
{
    ...
    crfsdn_rdlock(CHFS_MD_DN(chfs_md), LOC_CHFS_0110);
    crfsdn_print(log, CHFS_MD_DN(chfs_md));
    crfsdn_unlock(CHFS_MD_DN(chfs_md), LOC_CHFS_0111);
...
EC_BOOL chfs_show_cached_np(const UINT32 chfs_md_id, LOG *log)
{
    ...
    chfsnp_mgr_rdlock(CHFS_MD_NPP(chfs_md), LOC_CHFS_0112); 
    if(EC_FALSE == chfsnp_mgr_show_cached_np(log, CHFS_MD_NPP(chfs_md)))
    {
        chfsnp_mgr_unlock(CHFS_MD_NPP(chfs_md), LOC_CHFS_0113);
...
EC_BOOL chfs_show_specific_np(const UINT32 chfs_md_id, const UINT32 chfsnp_id, LOG *log)
{
    ...
    chfsnp_mgr_rdlock(CHFS_MD_NPP(chfs_md), LOC_CHFS_0115); 
    if(EC_FALSE == chfsnp_mgr_show_np(log, CHFS_MD_NPP(chfs_md), (uint32_t)chfsnp_id))
    {
        chfsnp_mgr_unlock(CHFS_MD_NPP(chfs_md), LOC_CHFS_0116);
...
static EC_BOOL __chfs_retire_of_np(const UINT32 chfs_md_id, const uint32_t chfsnp_id, const UINT32 nsec, const UINT32 expect_retire_num, const UINT32 max_step, UINT32 *complete_retire_num)
{
    CHFS_MD      *chfs_md;
...
static EC_BOOL __chfs_recycle_of_np(const UINT32 chfs_md_id, const uint32_t chfsnp_id, const UINT32 max_num, UINT32 *complete_num)
{
    CHFS_MD      *chfs_md;
