...
CPGV *cpgv_new(const uint8_t *cpgv_fname)
{
    ...
    CPGV_FSIZE(cpgv) = CPGV_HDR_SIZE;
    dbg_log(SEC_0074_CPGV, 9)(LOGSTDOUT, "[DEBUG] CPGV_HDR_SIZE = %ld\n", CPGV_HDR_SIZE);
...
STATIC_CAST static EC_BOOL __cpgv_del_disk(CPGV *cpgv, const uint16_t disk_no, const uint16_t page_model)
{
    ...
    if(EC_FALSE == cpgrb_tree_delete_data(CPGV_PAGE_DISK_CPGRB_POOL(cpgv), &(CPGV_PAGE_MODEL_DISK_CPGRB_ROOT_POS(cpgv, page_model)), disk_no))
    {
...
EC_BOOL cpgv_load(CPGV *cpgv, int fd, UINT32 *offset)
{
    ...
    if(NULL_PTR == CPGV_HEADER(cpgv))
    {
        CPGV_HEADER(cpgv) = safe_malloc(CPGV_HDR_SIZE, LOC_CPGV_0019);
        if(NULL_PTR == CPGV_HEADER(cpgv))
        {
