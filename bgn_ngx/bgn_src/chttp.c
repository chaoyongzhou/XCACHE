...
EC_BOOL chttp_store_init(CHTTP_STORE *chttp_store)
{
    if(NULL_PTR != chttp_store)
    {
        ...
        CHTTP_STORE_CACHE_DONE(chttp_store)        = CHTTP_STORE_CACHE_ERR;
...
EC_BOOL chttp_store_clone(const CHTTP_STORE *chttp_store_src, CHTTP_STORE *chttp_store_des)
{
    if(NULL_PTR != chttp_store_des)
    {
        ...
        CHTTP_STORE_CACHE_DONE(chttp_store_des)         = CHTTP_STORE_CACHE_DONE(chttp_store_src);
...
{
    static char space[ CHTTP_STAT_LOG_BUFF_MAX_SIZE ];

    if(NULL_PTR != chttp_stat)
    {
        char       *buff;
        uint32_t    buff_max_len;
        uint32_t    buff_len;
        uint64_t    log_bitmap;

        buff            = (char *)space;
        buff_max_len    = CHTTP_STAT_LOG_BUFF_MAX_SIZE;
        buff_len        = 0;
        log_bitmap      = CHTTP_STAT_LOG_BITMAP(chttp_stat);

        if(CHTTP_STAT_LOG_000_RSVD & log_bitmap)
        {
            buff_len += snprintf(buff + buff_len, buff_max_len - buff_len, "%s\t", "-");
        }

        if(CHTTP_STAT_LOG_001_NODE_LAYER & log_bitmap)
        {
            buff_len += snprintf(buff + buff_len, buff_max_len - buff_len, "%s\t", "proxy");
        }
        ...
        if(CHTTP_STAT_LOG_006_ORIG_PROTOCOL & log_bitmap)
        {
            buff_len += snprintf(buff + buff_len, buff_max_len - buff_len, "%s\t", "http");
        }

        if(CHTTP_STAT_LOG_007_ORIG_HOST & log_bitmap)
        {
            buff_len += snprintf(buff + buff_len, buff_max_len - buff_len, "%s\t",
                                    (char *)CHTTP_STAT_REQ_HOST(chttp_stat));
        }
        ...
        if(CHTTP_STAT_LOG_011_ORIG_RSP_REACH_MSEC & log_bitmap)
        {
            uint32_t    msec;

            msec = (uint32_t)(CHTTP_STAT_RSP_S_MSEC(chttp_stat) - CHTTP_STAT_REQ_S_MSEC(chttp_stat));
            buff_len += snprintf(buff + buff_len, buff_max_len - buff_len, "%u\t", msec);
        }

        if(CHTTP_STAT_LOG_012_DEV_NAME & log_bitmap)
        {
            buff_len += snprintf(buff + buff_len, buff_max_len - buff_len, "%s\t",
                                    (char *)CHTTP_STAT_DEVICE_NAME(chttp_stat));
        }
        ...
        if(CHTTP_STAT_LOG_022_ORIG_SENT_NBYTES & log_bitmap)
        {
            buff_len += snprintf(buff + buff_len, buff_max_len - buff_len, "%u\t",
                                    CHTTP_STAT_S_SEND_LEN(chttp_stat));
        }

        if(CHTTP_STAT_LOG_023_TRACE_ID & log_bitmap)
        {
            buff_len += snprintf(buff + buff_len, buff_max_len - buff_len, "%s\t",
                                    (char *)CHTTP_STAT_TRACE_ID(chttp_stat));
        }

        if(0 < buff_len && CHTTP_STAT_LOG_BUFF_MAX_SIZE > buff_len)
        {
            sys_print(log, "%.*s\n", buff_len, buff);
        }
    }
...
EC_BOOL chttp_node_init(CHTTP_NODE *chttp_node, const UINT32 type)
{
    if(NULL_PTR != chttp_node)
    {
        ...
        ccallback_node_init(CHTTP_NODE_PARSE_ON_MESSAGE_BEGIN_CB(chttp_node));
        ccallback_node_init(CHTTP_NODE_PARSE_ON_HEADERS_COMPLETE_CB(chttp_node));
        ccallback_node_init(CHTTP_NODE_PARSE_ON_BODY_CB(chttp_node));
        ccallback_node_init(CHTTP_NODE_PARSE_ON_MESSAGE_COMPLETE_CB(chttp_node));
...
EC_BOOL chttp_node_clean(CHTTP_NODE *chttp_node)
{
    if(NULL_PTR != chttp_node)
    {
        ...
        ccallback_node_clean((CHTTP_NODE_PARSE_ON_MESSAGE_BEGIN_CB(chttp_node)));
        ccallback_node_clean((CHTTP_NODE_PARSE_ON_HEADERS_COMPLETE_CB(chttp_node)));
        ccallback_node_clean((CHTTP_NODE_PARSE_ON_BODY_CB(chttp_node)));
        ccallback_node_clean((CHTTP_NODE_PARSE_ON_MESSAGE_COMPLETE_CB(chttp_node)));
...
EC_BOOL chttp_node_reserve(CHTTP_NODE *chttp_node)
{
    ...
    dbg_log(SEC_0149_CHTTP, 9)(LOGSTDOUT, "[DEBUG] chttp_node_reserve: reserve chttp_node %p, error %s, counter %u\n",
                                          chttp_node,
...
EC_BOOL chttp_node_release(CHTTP_NODE *chttp_node)
{
    ...
    dbg_log(SEC_0149_CHTTP, 9)(LOGSTDOUT, "[DEBUG] chttp_node_release: reserve chttp_node %p, error %s, counter %u\n",
                                          chttp_node,
                                          c_bit_bool_str(CHTTP_NODE_IS_ERROR(chttp_node)),
                                          CHTTP_NODE_COUNTER(chttp_node));
...
EC_BOOL chttp_node_wait_resume(CHTTP_NODE *chttp_node)
{
    ...
    if(NULL_PTR != CHTTP_NODE_CSOCKET_CNODE(chttp_node))
    {
        ...
        dbg_log(SEC_0149_CHTTP, 5)(LOGSTDOUT, "[DEBUG] chttp_node_wait_resume:"
                          "[no keep-alive] unbind csocket_cnode %p (sockfd %d, reusing %u) from chttp_node %p\n",
...
EC_BOOL chttp_node_recv_req(CHTTP_NODE *chttp_node)
{
    ...
    ret = chttp_node_recv(chttp_node);
    if(EC_FALSE == ret)
    {
...
EC_BOOL chttp_node_send_req(CHTTP_NODE *chttp_node)
{
    ...
    if(0 == CHTTP_NODE_REQ_S_MSEC(chttp_node))
    {
        /*record 1st sending time*/
        CHTTP_NODE_LOG_REQ_S_TIME(chttp_node);
    }
    ...
    if(EC_FALSE == chttp_node_send(chttp_node))
    {
    ...
    CHTTP_NODE_LOG_REQ_E_TIME(chttp_node);
...
EC_BOOL chttp_node_recv_rsp(CHTTP_NODE *chttp_node)
{
    ...
    if(EC_AGAIN == ret && BIT_FALSE == CSOCKET_CNODE_NONBLOCK(csocket_cnode)) /*block mode*/
    {
        ...
        chttp_node_release(chttp_node);
        return chttp_node_recv_rsp(chttp_node);
    }
...
EC_BOOL chttp_node_complete(CHTTP_NODE *chttp_node)
{
    ...
    if(CHTTP_TYPE_DO_SRV_REQ == CHTTP_NODE_TYPE(chttp_node))/*on server side*/
    {
        ...
        dbg_log(SEC_0149_CHTTP, 5)(LOGSTDOUT, "[DEBUG] chttp_node_complete:"
    ...
    {
        CHTTP_NODE_LOG_REQ_C_TIME(chttp_node);
...
EC_BOOL chttp_node_close(CHTTP_NODE *chttp_node)
{
    ...
    if(CHTTP_TYPE_DO_SRV_REQ == CHTTP_NODE_TYPE(chttp_node))/*on server side*/
    {
        ...
        if(BIT_TRUE == CHTTP_NODE_KEEPALIVE(chttp_node))
        {
            dbg_log(SEC_0149_CHTTP, 9)(LOGSTDOUT, "[DEBUG] chttp_node_close: "
                            "[server] keep-alive, resume sockfd %d\n",
                            CSOCKET_CNODE_SOCKFD(csocket_cnode));
        ...
        dbg_log(SEC_0149_CHTTP, 5)(LOGSTDOUT, "[DEBUG] chttp_node_close:"
                          "[DO_SRV_REQ] unbind csocket_cnode %p (sockfd %d, reusing %u) from chttp_node %p\n",
                          csocket_cnode, CSOCKET_CNODE_SOCKFD(csocket_cnode), CSOCKET_CNODE_REUSING(csocket_cnode),
                          chttp_node);
    ...
    dbg_log(SEC_0149_CHTTP, 0)(LOGSTDOUT, "error:chttp_node_close:"
...
EC_BOOL chttp_node_timeout(CHTTP_NODE *chttp_node)
{
    ...
    {
        dbg_log(SEC_0149_CHTTP, 0)(LOGSTDOUT, "[DEBUG] chttp_node_timeout: "
    ...
    if(CHTTP_TYPE_DO_CLT_RSP == CHTTP_NODE_TYPE(chttp_node)) /*client side*/
    {
        ...
        dbg_log(SEC_0149_CHTTP, 5)(LOGSTDOUT, "[DEBUG] chttp_node_timeout:"
                          "[CLT_RSP] unbind csocket_cnode %p (sockfd %d, reusing %u) from chttp_node %p\n",
                          csocket_cnode, CSOCKET_CNODE_SOCKFD(csocket_cnode), CSOCKET_CNODE_REUSING(csocket_cnode),
                          chttp_node);
    ...
    dbg_log(SEC_0149_CHTTP, 0)(LOGSTDOUT, "error:chttp_node_timeout:"
                    "should never reach here, release chttp_node and try to close sockfd %d\n",
                    CSOCKET_CNODE_SOCKFD(csocket_cnode));
...
EC_BOOL chttp_node_shutdown(CHTTP_NODE *chttp_node)
{
    ...
    if(CHTTP_TYPE_DO_SRV_REQ == CHTTP_NODE_TYPE(chttp_node)) /*server side*/
    {
        dbg_log(SEC_0149_CHTTP, 9)(LOGSTDOUT, "[DEBUG] chttp_node_shutdown: "
        ...
        dbg_log(SEC_0149_CHTTP, 5)(LOGSTDOUT, "[DEBUG] chttp_node_shutdown:"
                          "[DO_SRV_REQ] unbind csocket_cnode %p (sockfd %d, reusing %u) from chttp_node %p\n",
                          csocket_cnode, CSOCKET_CNODE_SOCKFD(csocket_cnode), CSOCKET_CNODE_REUSING(csocket_cnode),
                          chttp_node);
    ...
    dbg_log(SEC_0149_CHTTP, 0)(LOGSTDOUT, "error:chttp_node_shutdown:"
                    "should never reach here, release chttp_node and try to close sockfd %d\n",
                    CSOCKET_CNODE_SOCKFD(csocket_cnode));
...
EC_BOOL chttp_node_parse_on_message_begin(CHTTP_NODE *chttp_node)
{
    if(NULL_PTR != chttp_node)
    {
        CHTTP_NODE_LOG_RSP_S_TIME(chttp_node);
    }
    return (EC_TRUE);
}
...
EC_BOOL chttp_node_parse_on_message_complete(CHTTP_NODE *chttp_node)
{
    ...
    && NULL_PTR != CHTTP_NODE_STORE(chttp_node))
    {
        ...
        CHTTP_NODE_LOG_RSP_E_TIME(chttp_node);
...
EC_BOOL chttp_node_adjust_seg_id(CHTTP_NODE *chttp_node)
{
    ...
    if(EC_TRUE == chttp_node_is_chunked(chttp_node))
    {
        dbg_log(SEC_0149_CHTTP, 9)(LOGSTDOUT, "[DEBUG] chttp_node_adjust_seg_id: "
                                              "chunked, adjust seg_id %u => 0\n",
                                              CHTTP_STORE_SEG_ID(chttp_store));
...
EC_BOOL chttp_srv_accept_once(CSRV *csrv, EC_BOOL *continue_flag)
{
    ...
    if(EC_TRUE == ret)
    {
        ...
        dbg_log(SEC_0149_CHTTP, 5)(LOGSTDOUT, "[DEBUG] chttp_srv_accept_once:"
        ...
        chttp_node_set_socket_callback(chttp_node);
        chttp_node_set_socket_epoll(chttp_node);
...
EC_BOOL chttp_node_set_socket_callback(CHTTP_NODE *chttp_node)
{
    ...
    if(CHTTP_TYPE_DO_CLT_RSP == CHTTP_NODE_TYPE(chttp_node))
    {
        ...
        csocket_cnode_set_recv_callback(csocket_cnode,
                                         ...
                                         (void *)chttp_node,
                                         (void *)chttp_node_recv_rsp);

        csocket_cnode_set_send_callback(csocket_cnode,
                                         (const char *)"chttp_node_send_req",
                                         (void *)chttp_node,
                                         (void *)chttp_node_send_req);
...
EC_BOOL chttp_node_connect(CHTTP_NODE *chttp_node, const UINT32 csocket_block_mode, const UINT32 ipaddr, const UINT32 port)
{
    ...
    dbg_log(SEC_0149_CHTTP, 5)(LOGSTDOUT, "[DEBUG] chttp_connect:"
                      "bind csocket_cnode %p (sockfd %d, reusing %u) to chttp_node %p\n",
                      csocket_cnode, CSOCKET_CNODE_SOCKFD(csocket_cnode), CSOCKET_CNODE_REUSING(csocket_cnode),
                      chttp_node);
    return (EC_TRUE);
...
EC_BOOL chttp_node_disconnect(CHTTP_NODE *chttp_node)
{
    if(NULL_PTR != CHTTP_NODE_CSOCKET_CNODE(chttp_node))
    {
        ...
        dbg_log(SEC_0149_CHTTP, 5)(LOGSTDOUT, "[DEBUG] chttp_node_disconnect:"
...
EC_BOOL chttp_node_detach(CHTTP_NODE *chttp_node)
{
    ...
    if(0 < rsp_body_len && 0 < CHTTP_NODE_CONTENT_LENGTH(chttp_node))
    {
        ...
        {
            dbg_log(SEC_0149_CHTTP, 0)(LOGSTDOUT, "error:chttp_node_detach: "
                                                  "body len %"PRId64" != content len %"PRId64"\n",
    ...
    dbg_log(SEC_0149_CHTTP, 9)(LOGSTDOUT, "[DEBUG] chttp_node_detach: "
                                          "body len %"PRId64", content len %"PRId64"\n",
    ...
    if(do_log(SEC_0149_CHTTP, 9))
    {
        sys_log(LOGSTDOUT, "[DEBUG] chttp_node_detach: "
                           "before handover, chttp_node: %p\n",
                           chttp_node);
...
EC_BOOL chttp_node_renew_content_length(CHTTP_NODE *chttp_node, CHTTP_STORE *chttp_store, const uint64_t content_length)
{
    ...
    dbg_log(SEC_0149_CHTTP, 5)(LOGSTDOUT, "[DEBUG] chttp_node_renew_content_length: "
                                          "remove chunked header done\n");
...
EC_BOOL chttp_node_store_on_headers_complete(CHTTP_NODE *chttp_node)
{
    ...
    )
    {
        ...
        CHTTP_STORE_CACHE_DONE(chttp_store) |= CHTTP_STORE_CACHE_HEADER;
    }
...
EC_BOOL chttp_node_store_on_message_complete(CHTTP_NODE *chttp_node)
{
    ...
    )
    {
        ...
        CHTTP_STORE_CACHE_DONE(chttp_store) |= CHTTP_STORE_CACHE_WHOLE;
    }
...
{
    if(NULL_PTR != chttp_node && NULL_PTR != chttp_req)
    {
        CHTTP_STAT      *chttp_stat;

        chttp_stat = CHTTP_NODE_STAT(chttp_node);

        if(EC_FALSE == cstring_is_empty(CHTTP_REQ_DEVICE_NAME(chttp_req)))
        {
            char     *device_name;

            device_name = (char *)cstring_get_str(CHTTP_REQ_DEVICE_NAME(chttp_req));
            CHTTP_STAT_SET_DEVICE_NAME(chttp_stat, device_name);

            dbg_log(SEC_0149_CHTTP, 9)(LOGSTDOUT, "[DEBUG] chttp_node_import_req: "
                                                  "import device name '%s' to node\n",
                                                  device_name);
        }

        if(EC_FALSE == cstring_is_empty(CHTTP_REQ_TRACE_ID(chttp_req)))
        {
            char     *trace_id;

            trace_id = (char *)cstring_get_str(CHTTP_REQ_TRACE_ID(chttp_req));
            CHTTP_STAT_SET_TRACE_ID(chttp_stat, trace_id);

            dbg_log(SEC_0149_CHTTP, 9)(LOGSTDOUT, "[DEBUG] chttp_node_import_req: "
                                                  "import trace id '%s' to node\n",
                                                  trace_id);
        }
    }

    /*switch on all log putput*/
    if(NULL_PTR != chttp_node)
    {
        CHTTP_STAT      *chttp_stat;
        ...
        CHTTP_STAT_LOG_BITMAP(chttp_stat) = (~((uint64_t)0));
    }
    return (EC_TRUE);
}
...
EC_BOOL chttp_request_block(const CHTTP_REQ *chttp_req, CHTTP_RSP *chttp_rsp, CHTTP_STAT *chttp_stat)
{
     ...
     )
    {
        ...
        dbg_log(SEC_0149_CHTTP, 5)(LOGSTDOUT, "[DEBUG] chttp_request_block:"
    ...
    for(;;)
    {
        ret = chttp_node_send_req(chttp_node);
        if(EC_AGAIN != ret)
        {
            break;
        }
    ...
    {
        ret = chttp_node_recv_rsp(chttp_node);
    }
...
EC_BOOL chttp_request_basic(const CHTTP_REQ *chttp_req, CHTTP_STORE *chttp_store, CHTTP_RSP *chttp_rsp, CHTTP_STAT *chttp_stat)
{
    ...
    if(EC_FALSE == chttp_node_connect(chttp_node, CSOCKET_IS_NONBLOCK_MODE,
                                        CHTTP_REQ_IPADDR(chttp_req), CHTTP_REQ_PORT(chttp_req)))
    {
     ...
     )
    {
        ...
        dbg_log(SEC_0149_CHTTP, 5)(LOGSTDOUT, "[DEBUG] chttp_request_basic:"
                          "[1] unbind csocket_cnode %p (sockfd %d, reusing %u) from chttp_node %p\n",
                          csocket_cnode, CSOCKET_CNODE_SOCKFD(csocket_cnode), CSOCKET_CNODE_REUSING(csocket_cnode),
                          chttp_node);
    ...
    __COROUTINE_IF_EXCEPTION() {/*exception*/
        ...
        && EC_TRUE == chttp_node_is_chunked(chttp_node))
        {
            ...
            {
                dbg_log(SEC_0149_CHTTP, 0)(LOGSTDOUT, "error:chttp_request_basic: "
                                                      "croutine load for chttp_node_detach failed\n");
            ...
            dbg_log(SEC_0149_CHTTP, 9)(LOGSTDOUT, "[DEBUG] chttp_request_basic: "
                                                  "chunked => detach flow, co %p\n",
                                                  croutine_node);
...
EC_BOOL chttp_node_check(CHTTP_NODE *chttp_node, const UINT32 ipaddr, const UINT32 port)
{
    ...
    dbg_log(SEC_0149_CHTTP, 5)(LOGSTDOUT, "[DEBUG] chttp_node_check:"
                      "bind csocket_cnode %p (sockfd %d, reusing %u) to chttp_node %p\n",
                      csocket_cnode, CSOCKET_CNODE_SOCKFD(csocket_cnode), CSOCKET_CNODE_REUSING(csocket_cnode),
                      chttp_node);
...
EC_BOOL chttp_request_merge(const CHTTP_REQ *chttp_req, CHTTP_STORE *chttp_store, CHTTP_RSP *chttp_rsp, CHTTP_STAT *chttp_stat)
{
...
#if 0 /*merge orig procedure would fetch seg, here read operation is not necessary*/
    ...
    if(0 == CHTTP_STORE_SEG_ID(chttp_store_t) && BIT_TRUE == CHTTP_STORE_EXPIRED_FLAG(chttp_store_t))
    {
        if(EC_FALSE == __chttp_request_merge_file_wait_ready(chttp_req, chttp_store_t, &path, chttp_rsp, chttp_stat))
        {
            ...
            __chttp_request_merge_file_unlock(chttp_req, chttp_store_t, &path); /*unlock when exception happen*/
    ...
    else
    {
        ...
        if(EC_TRUE == data_ready)
        {
            ...
            __chttp_request_merge_file_unlock(chttp_req, chttp_store_t, &path); /*for safe reason*/
...
STATIC_CAST static EC_BOOL __chttp_request_header_file_wait(const CHTTP_REQ *chttp_req, const CHTTP_STORE *chttp_store, const CSTRING *path, CHTTP_RSP *chttp_rsp, CHTTP_STAT *chttp_stat, UINT32 *data_ready)
{
    ...
    if(EC_FALSE == (*data_ready))
    {
        ...
        if(EC_FALSE == super_cond_wait(0, tag, path, timeout_msec))
        {
            dbg_log(SEC_0149_CHTTP, 0)(LOGSTDOUT, "error:__chttp_request_header_file_wait: cond wait '%s' failed\n",
                        (char *)cstring_get_str(path));
            return (EC_FALSE);
        }
        dbg_log(SEC_0149_CHTTP, 9)(LOGSTDOUT, "[DEBUG] __chttp_request_header_file_wait: cond wait '%s' <= back\n",
                        (char *)cstring_get_str(path));
    }
    ...
    if(EC_FALSE == ccache_file_read(store_srv_tcid, store_srv_ipaddr, store_srv_port,
                    path, CHTTP_SEG_ERR_OFFSET, CHTTP_SEG_ERR_OFFSET, &content_cbytes))
    {
