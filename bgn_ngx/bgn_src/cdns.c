...
STATIC_CAST static EC_BOOL __cdns_on_recv_complete(CDNS_NODE *cdns_node)
{
...
EC_BOOL cdns_node_recv_rsp(CDNS_NODE *cdns_node)
{
    ...
    if(NULL_PTR != CDNS_NODE_SET_CALLBACK_FUNC(cdns_node))
    {
        dbg_log(SEC_0150_CDNS, 9)(LOGSTDOUT, "[DEBUG] cdns_node_recv_rsp: return EC_TRUE\n");
        return (EC_TRUE);
    }

    dbg_log(SEC_0150_CDNS, 9)(LOGSTDOUT, "[DEBUG] cdns_node_recv_rsp: return EC_DONE\n");
    return (EC_DONE);
}
...
EC_BOOL cdns_node_create(CDNS_NODE *cdns_node, const CDNS_REQ * cdns_req)
{
    ...
    csocket_cnode = csocket_cnode_new(LOC_CDNS_0006);
    if(NULL_PTR == csocket_cnode)
    {
...
EC_BOOL cdns_make_header(CDNS_NODE *cdns_node, CDNS_HEADER *cdns_header)
{
    ...
    flag = (flag << 1) | CDNS_HEADER_FLAG_RA(cdns_header);

    flag = (flag << 3) | CDNS_HEADER_FLAG_ZERO(cdns_header);
...
EC_BOOL cdns_parse_host(CDNS_NODE *cdns_node, const uint32_t max_len, uint32_t *pos, CSTRING *host)
{
    ...
    cstring_expand_to(host, len, LOC_CDNS_0007);
...
EC_BOOL cdns_parse_rsp(CDNS_NODE *cdns_node, CDNS_RSP *cdns_rsp)
{
    ...
    dbg_log(SEC_0150_CDNS, 9)(LOGSTDOUT, "[DEBUG] cdns_parse_rsp: sockfd %d parse rsp done\n", CSOCKET_CNODE_SOCKFD(csocket_cnode));
...
CDNS_REQ *cdns_req_new()
{
    ...
    alloc_static_mem(MM_CDNS_REQ, &cdns_req, LOC_CDNS_0009);
...
EC_BOOL cdns_req_free(CDNS_REQ *cdns_req)
{
    if(NULL_PTR != cdns_req)
    {
        ...
        free_static_mem(MM_CDNS_REQ, cdns_req, LOC_CDNS_0011);
    }
...
CDNS_RSP *cdns_rsp_new()
{
    ...
    alloc_static_mem(MM_CDNS_RSP, &cdns_rsp, LOC_CDNS_0012);
...
{
    clist_init(cdns_rsp, MM_CDNS_RSP_NODE, LOC_CDNS_0014);
...
EC_BOOL cdns_rsp_free(CDNS_RSP *cdns_rsp)
{
    if(NULL_PTR != cdns_rsp)
    {
        ...
        free_static_mem(MM_CDNS_RSP, cdns_rsp, LOC_CDNS_0015);
    }
...
CDNS_RSP_NODE *cdns_rsp_node_new()
{
    ...
    alloc_static_mem(MM_CDNS_RSP_NODE, &cdns_rsp_node, LOC_CDNS_0016);
    if(NULL_PTR == cdns_rsp_node)
    {
...
EC_BOOL cdns_rsp_node_free(CDNS_RSP_NODE *cdns_rsp_node)
{
    if(NULL_PTR != cdns_rsp_node)
    {
        ...
        free_static_mem(MM_CDNS_RSP_NODE, cdns_rsp_node, LOC_CDNS_0018);
    }
...
EC_BOOL cdns_request_basic(const CDNS_REQ *cdns_req, CDNS_NODE_SET_CALLBACK handler, void *private_data0, void *private_data1)
{
    ...
    if(NULL_PTR == cdns_rsp)
    {
        dbg_log(SEC_0150_CDNS, 0)(LOGSTDOUT, "error:cdns_request_basic: new cdns_rsp failed\n");
        return (EC_FALSE);
    }
    ...
    if(NULL_PTR == cdns_node)
    {
        dbg_log(SEC_0150_CDNS, 0)(LOGSTDOUT, "error:cdns_request_basic: new cdns_node failed\n");
        cdns_rsp_free(cdns_rsp);
        return (EC_FALSE);
    }
    ...
    {
        dbg_log(SEC_0150_CDNS, 0)(LOGSTDOUT, "error:cdns_request_basic: connect server %s:%ld failed\n",
...
EC_BOOL cdns_request(const CDNS_REQ *cdns_req, CDNS_RSP *cdns_rsp)
{
    ...
    croutine_cond_reserve(croutine_cond, 1, LOC_CDNS_0020);
    croutine_cond_wait(croutine_cond, LOC_CDNS_0021);
