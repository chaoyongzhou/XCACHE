...
EC_BOOL ctdnshttp_commit_request(CHTTP_NODE *chttp_node)
{
    ...
    if(CHTTP_METHOD_UNKNOWN != method)
    {
        ...
        croutine_node = croutine_pool_load_preempt(TASK_REQ_CTHREAD_POOL(task_brd_default_get()),
                                           (UINT32)ctdnshttp_commit_start, 2, chttp_node, (UINT32)method);
        if(NULL_PTR == croutine_node)
        {
            dbg_log(SEC_0048_CTDNSHTTP, 0)(LOGSTDOUT, "error:ctdnshttp_commit_request: "
                                                      "no croutine\n");
...
EC_BOOL ctdnshttp_commit_start(CHTTP_NODE *chttp_node, const UINT32 method)
{
    ...
    if(NULL_PTR == chttp_api)
    {
        ...
        dbg_log(SEC_0048_CTDNSHTTP, 0)(LOGSTDOUT, "error:ctdnshttp_commit_start: "
                                                  "no api for %s:'%.*s'\n",
        ...
        CHTTP_NODE_LOG_STAT_WHEN_DONE(chttp_node, "TDNS_ERR %u --", CHTTP_NOT_ACCEPTABLE);
        CHTTP_NODE_LOG_INFO_WHEN_DONE(chttp_node, "error:ctdnshttp_commit_start: invalid request %s:'%.*s'",
                                                  chttp_method_str((uint32_t)method),
    ...
    dbg_log(SEC_0048_CTDNSHTTP, 9)(LOGSTDOUT, "[DEBUG] ctdnshttp_commit_start: "
                                              "api: method %d, name %s\n",
