...
static const CHTTP_API g_cngx_https_api_list[] = {
    ...
    {CONST_STR_AND_LEN("rfs_up")          , CHTTP_METHOD_GET  , cngx_https_commit_rfs_up_request},
    {CONST_STR_AND_LEN("rfs_down")        , CHTTP_METHOD_GET  , cngx_https_commit_rfs_down_request},
    {CONST_STR_AND_LEN("rfs_add")         , CHTTP_METHOD_GET  , cngx_https_commit_rfs_add_request},
    {CONST_STR_AND_LEN("rfs_del")         , CHTTP_METHOD_GET  , cngx_https_commit_rfs_del_request},
    {CONST_STR_AND_LEN("rfs_list")        , CHTTP_METHOD_GET  , cngx_https_commit_rfs_list_request},
...
EC_BOOL cngx_https_commit_rfs_up_request(CHTTP_NODE *chttp_node)
{
    ...
    if(EC_FALSE == cngx_https_handle_rfs_up_request(chttp_node))
    {
        dbg_log(SEC_0056_CNGX_HTTPS, 0)(LOGSTDOUT, "error:cngx_https_commit_rfs_up_request: handle request failed\n");
        return (EC_FALSE);
    }

    if(EC_FALSE == cngx_https_make_rfs_up_response(chttp_node))
    {
        dbg_log(SEC_0056_CNGX_HTTPS, 0)(LOGSTDOUT, "error:cngx_https_commit_rfs_up_request: make response failed\n");
        return (EC_FALSE);
    }

    ret = cngx_https_commit_rfs_up_response(chttp_node);
    if(EC_FALSE == ret)
    {
        dbg_log(SEC_0056_CNGX_HTTPS, 0)(LOGSTDOUT, "error:cngx_https_commit_rfs_up_request: commit response failed\n");
        return (EC_FALSE);
    }
...
EC_BOOL cngx_https_handle_rfs_up_request(CHTTP_NODE *chttp_node)
{
    ...
    if(!(0 == req_body_chunk_num))
    {
        ...
        dbg_log(SEC_0056_CNGX_HTTPS, 0)(LOGSTDOUT, "error: cngx_https_handle_rfs_up_request: chunk num %ld\n", req_body_chunk_num);

        dbg_log(SEC_0056_CNGX_HTTPS, 0)(LOGSTDOUT, "error: cngx_https_handle_rfs_up_request: chunk mgr %p info\n", req_body_chunks);
        chunk_mgr_print_info(LOGSTDOUT, req_body_chunks);

        dbg_log(SEC_0056_CNGX_HTTPS, 0)(LOGSTDOUT, "error: cngx_https_handle_rfs_up_request: chunk mgr %p chars\n", req_body_chunks);
        chunk_mgr_print_chars(LOGSTDOUT, req_body_chunks);
    ...
    if(1)
    {
        ...
        if(NULL_PTR != v)
        {
            ...
            CMON_NODE_TCID(&cmon_node) = c_ipv4_to_word(v);
            dbg_log(SEC_0056_CNGX_HTTPS, 1)(LOGSTDOUT, "[DEBUG] cngx_https_handle_rfs_up_request: header rfs-tcid %s => 0x%lx\n",
                                v, CMON_NODE_TCID(&cmon_node));
        }
        ...
        if(NULL_PTR != v)
        {
            ...
            CMON_NODE_IPADDR(&cmon_node) = c_ipv4_to_word(v);
            dbg_log(SEC_0056_CNGX_HTTPS, 1)(LOGSTDOUT, "[DEBUG] cngx_https_handle_rfs_up_request: header rfs-ip %s => 0x%lx\n",
                                v, CMON_NODE_IPADDR(&cmon_node));
        }
        ...
        {
            CMON_NODE_PORT(&cmon_node) = c_str_to_word(v);
            dbg_log(SEC_0056_CNGX_HTTPS, 1)(LOGSTDOUT, "[DEBUG] cngx_https_handle_rfs_up_request: header rfs-port %s => %ld\n",
                                v, CMON_NODE_PORT(&cmon_node));
        }
        ...
        {
            CMON_NODE_MODI(&cmon_node) = c_str_to_word(v);
            dbg_log(SEC_0056_CNGX_HTTPS, 1)(LOGSTDOUT, "[DEBUG] cngx_https_handle_rfs_up_request: header rfs-modi %s => %ld\n",
                                v, CMON_NODE_MODI(&cmon_node));
        }
...
EC_BOOL cngx_https_make_rfs_up_response(CHTTP_NODE *chttp_node)
{
    if(EC_FALSE == chttp_make_response_header_common(chttp_node, (uint64_t)0))
    {
        dbg_log(SEC_0056_CNGX_HTTPS, 0)(LOGSTDOUT, "error:cngx_https_make_rfs_up_response: make response header failed\n");
        return (EC_FALSE);
    }
    ...
    if(BIT_TRUE == CHTTP_NODE_KEEPALIVE(chttp_node))
    {
        if(EC_FALSE == chttp_make_response_header_keepalive(chttp_node))
        {
            dbg_log(SEC_0056_CNGX_HTTPS, 0)(LOGSTDOUT, "error:cngx_https_make_rfs_up_response: make response header keepalive failed\n");
            return (EC_FALSE);
        }
    }
    ...
    if(EC_FALSE == chttp_make_response_header_end(chttp_node))
    {
        dbg_log(SEC_0056_CNGX_HTTPS, 0)(LOGSTDOUT, "error:cngx_https_make_rfs_up_response: make header end failed\n");
        return (EC_FALSE);
    }
...
EC_BOOL cngx_https_commit_rfs_up_response(CHTTP_NODE *chttp_node)
{
    ...
    if(NULL_PTR == csocket_cnode)
    {
        dbg_log(SEC_0056_CNGX_HTTPS, 0)(LOGSTDOUT, "error:cngx_https_commit_rfs_up_response: csocket_cnode of chttp_node %p is null\n", chttp_node);
        return (EC_FALSE);
    }
...
EC_BOOL cngx_https_commit_rfs_down_request(CHTTP_NODE *chttp_node)
{
    ...
    if(EC_FALSE == cngx_https_handle_rfs_down_request(chttp_node))
    {
        dbg_log(SEC_0056_CNGX_HTTPS, 0)(LOGSTDOUT, "error:cngx_https_commit_rfs_down_request: handle request failed\n");
        return (EC_FALSE);
    }

    if(EC_FALSE == cngx_https_make_rfs_down_response(chttp_node))
    {
        dbg_log(SEC_0056_CNGX_HTTPS, 0)(LOGSTDOUT, "error:cngx_https_commit_rfs_down_request: make response failed\n");
        return (EC_FALSE);
    }

    ret = cngx_https_commit_rfs_down_response(chttp_node);
    if(EC_FALSE == ret)
    {
        dbg_log(SEC_0056_CNGX_HTTPS, 0)(LOGSTDOUT, "error:cngx_https_commit_rfs_down_request: commit response failed\n");
        return (EC_FALSE);
    }
...
EC_BOOL cngx_https_handle_rfs_down_request(CHTTP_NODE *chttp_node)
{
    ...
    if(!(0 == req_body_chunk_num))
    {
        ...
        dbg_log(SEC_0056_CNGX_HTTPS, 0)(LOGSTDOUT, "error: cngx_https_handle_rfs_down_request: chunk num %ld\n", req_body_chunk_num);

        dbg_log(SEC_0056_CNGX_HTTPS, 0)(LOGSTDOUT, "error: cngx_https_handle_rfs_down_request: chunk mgr %p info\n", req_body_chunks);
        chunk_mgr_print_info(LOGSTDOUT, req_body_chunks);

        dbg_log(SEC_0056_CNGX_HTTPS, 0)(LOGSTDOUT, "error: cngx_https_handle_rfs_down_request: chunk mgr %p chars\n", req_body_chunks);
        chunk_mgr_print_chars(LOGSTDOUT, req_body_chunks);
    ...
    if(1)
    {
        ...
        if(NULL_PTR != v)
        {
            ...
            CMON_NODE_TCID(&cmon_node) = c_ipv4_to_word(v);
            dbg_log(SEC_0056_CNGX_HTTPS, 1)(LOGSTDOUT, "[DEBUG] cngx_https_handle_rfs_down_request: header rfs-tcid %s => 0x%lx\n",
                                v, CMON_NODE_TCID(&cmon_node));
        }
        ...
        if(NULL_PTR != v)
        {
            ...
            CMON_NODE_IPADDR(&cmon_node) = c_ipv4_to_word(v);
            dbg_log(SEC_0056_CNGX_HTTPS, 1)(LOGSTDOUT, "[DEBUG] cngx_https_handle_rfs_down_request: header rfs-ip %s => 0x%lx\n",
                                v, CMON_NODE_IPADDR(&cmon_node));
        }
        ...
        {
            CMON_NODE_PORT(&cmon_node) = c_str_to_word(v);
            dbg_log(SEC_0056_CNGX_HTTPS, 1)(LOGSTDOUT, "[DEBUG] cngx_https_handle_rfs_down_request: header rfs-port %s => %ld\n",
                                v, CMON_NODE_PORT(&cmon_node));
        }
        ...
        {
            CMON_NODE_MODI(&cmon_node) = c_str_to_word(v);
            dbg_log(SEC_0056_CNGX_HTTPS, 1)(LOGSTDOUT, "[DEBUG] cngx_https_handle_rfs_down_request: header rfs-modi %s => %ld\n",
                                v, CMON_NODE_MODI(&cmon_node));
        }
...
EC_BOOL cngx_https_make_rfs_down_response(CHTTP_NODE *chttp_node)
{
    if(EC_FALSE == chttp_make_response_header_common(chttp_node, (uint64_t)0))
    {
        dbg_log(SEC_0056_CNGX_HTTPS, 0)(LOGSTDOUT, "error:cngx_https_make_rfs_down_response: make response header failed\n");
        return (EC_FALSE);
    }
    ...
    if(BIT_TRUE == CHTTP_NODE_KEEPALIVE(chttp_node))
    {
        if(EC_FALSE == chttp_make_response_header_keepalive(chttp_node))
        {
            dbg_log(SEC_0056_CNGX_HTTPS, 0)(LOGSTDOUT, "error:cngx_https_make_rfs_down_response: make response header keepalive failed\n");
            return (EC_FALSE);
        }
    }
    ...
    if(EC_FALSE == chttp_make_response_header_end(chttp_node))
    {
        dbg_log(SEC_0056_CNGX_HTTPS, 0)(LOGSTDOUT, "error:cngx_https_make_rfs_down_response: make header end failed\n");
        return (EC_FALSE);
    }
...
EC_BOOL cngx_https_commit_rfs_down_response(CHTTP_NODE *chttp_node)
{
    ...
    if(NULL_PTR == csocket_cnode)
    {
        dbg_log(SEC_0056_CNGX_HTTPS, 0)(LOGSTDOUT, "error:cngx_https_commit_rfs_down_response: csocket_cnode of chttp_node %p is null\n", chttp_node);
        return (EC_FALSE);
    }
...
EC_BOOL cngx_https_commit_rfs_add_request(CHTTP_NODE *chttp_node)
{
    ...
    if(EC_FALSE == cngx_https_handle_rfs_add_request(chttp_node))
    {
        dbg_log(SEC_0056_CNGX_HTTPS, 0)(LOGSTDOUT, "error:cngx_https_commit_rfs_add_request: handle request failed\n");
        return (EC_FALSE);
    }

    if(EC_FALSE == cngx_https_make_rfs_add_response(chttp_node))
    {
        dbg_log(SEC_0056_CNGX_HTTPS, 0)(LOGSTDOUT, "error:cngx_https_commit_rfs_add_request: make response failed\n");
        return (EC_FALSE);
    }

    ret = cngx_https_commit_rfs_add_response(chttp_node);
    if(EC_FALSE == ret)
    {
        dbg_log(SEC_0056_CNGX_HTTPS, 0)(LOGSTDOUT, "error:cngx_https_commit_rfs_add_request: commit response failed\n");
        return (EC_FALSE);
    }
...
EC_BOOL cngx_https_handle_rfs_add_request(CHTTP_NODE *chttp_node)
{
    ...
    if(!(0 == req_body_chunk_num))
    {
        ...
        dbg_log(SEC_0056_CNGX_HTTPS, 0)(LOGSTDOUT, "error: cngx_https_handle_rfs_add_request: chunk num %ld\n", req_body_chunk_num);

        dbg_log(SEC_0056_CNGX_HTTPS, 0)(LOGSTDOUT, "error: cngx_https_handle_rfs_add_request: chunk mgr %p info\n", req_body_chunks);
        chunk_mgr_print_info(LOGSTDOUT, req_body_chunks);

        dbg_log(SEC_0056_CNGX_HTTPS, 0)(LOGSTDOUT, "error: cngx_https_handle_rfs_add_request: chunk mgr %p chars\n", req_body_chunks);
        chunk_mgr_print_chars(LOGSTDOUT, req_body_chunks);
    ...
    if(1)
    {
        ...
        if(NULL_PTR != v)
        {
            ...
            CMON_NODE_TCID(&cmon_node) = c_ipv4_to_word(v);
            dbg_log(SEC_0056_CNGX_HTTPS, 1)(LOGSTDOUT, "[DEBUG] cngx_https_handle_rfs_add_request: header rfs-tcid %s => 0x%lx\n",
                                v, CMON_NODE_TCID(&cmon_node));
        }
        ...
        if(NULL_PTR != v)
        {
            ...
            CMON_NODE_IPADDR(&cmon_node) = c_ipv4_to_word(v);
            dbg_log(SEC_0056_CNGX_HTTPS, 1)(LOGSTDOUT, "[DEBUG] cngx_https_handle_rfs_add_request: header rfs-ip %s => 0x%lx\n",
                                v, CMON_NODE_IPADDR(&cmon_node));
        }
        ...
        {
            CMON_NODE_PORT(&cmon_node) = c_str_to_word(v);
            dbg_log(SEC_0056_CNGX_HTTPS, 1)(LOGSTDOUT, "[DEBUG] cngx_https_handle_rfs_add_request: header rfs-port %s => %ld\n",
                                v, CMON_NODE_PORT(&cmon_node));
        }
        ...
        {
            CMON_NODE_MODI(&cmon_node) = c_str_to_word(v);
            dbg_log(SEC_0056_CNGX_HTTPS, 1)(LOGSTDOUT, "[DEBUG] cngx_https_handle_rfs_add_request: header rfs-modi %s => %ld\n",
                                v, CMON_NODE_MODI(&cmon_node));
        }
...
EC_BOOL cngx_https_make_rfs_add_response(CHTTP_NODE *chttp_node)
{
    if(EC_FALSE == chttp_make_response_header_common(chttp_node, (uint64_t)0))
    {
        dbg_log(SEC_0056_CNGX_HTTPS, 0)(LOGSTDOUT, "error:cngx_https_make_rfs_add_response: make response header failed\n");
        return (EC_FALSE);
    }
    ...
    if(BIT_TRUE == CHTTP_NODE_KEEPALIVE(chttp_node))
    {
        if(EC_FALSE == chttp_make_response_header_keepalive(chttp_node))
        {
            dbg_log(SEC_0056_CNGX_HTTPS, 0)(LOGSTDOUT, "error:cngx_https_make_rfs_add_response: make response header keepalive failed\n");
            return (EC_FALSE);
        }
    }
    ...
    if(EC_FALSE == chttp_make_response_header_end(chttp_node))
    {
        dbg_log(SEC_0056_CNGX_HTTPS, 0)(LOGSTDOUT, "error:cngx_https_make_rfs_add_response: make header end failed\n");
        return (EC_FALSE);
    }
...
EC_BOOL cngx_https_commit_rfs_add_response(CHTTP_NODE *chttp_node)
{
    ...
    if(NULL_PTR == csocket_cnode)
    {
        dbg_log(SEC_0056_CNGX_HTTPS, 0)(LOGSTDOUT, "error:cngx_https_commit_rfs_add_response: csocket_cnode of chttp_node %p is null\n", chttp_node);
        return (EC_FALSE);
    }
...
EC_BOOL cngx_https_commit_rfs_del_request(CHTTP_NODE *chttp_node)
{
    ...
    if(EC_FALSE == cngx_https_handle_rfs_del_request(chttp_node))
    {
        dbg_log(SEC_0056_CNGX_HTTPS, 0)(LOGSTDOUT, "error:cngx_https_commit_rfs_del_request: handle request failed\n");
        return (EC_FALSE);
    }

    if(EC_FALSE == cngx_https_make_rfs_del_response(chttp_node))
    {
        dbg_log(SEC_0056_CNGX_HTTPS, 0)(LOGSTDOUT, "error:cngx_https_commit_rfs_del_request: make response failed\n");
        return (EC_FALSE);
    }

    ret = cngx_https_commit_rfs_del_response(chttp_node);
    if(EC_FALSE == ret)
    {
        dbg_log(SEC_0056_CNGX_HTTPS, 0)(LOGSTDOUT, "error:cngx_https_commit_rfs_del_request: commit response failed\n");
        return (EC_FALSE);
    }
...
EC_BOOL cngx_https_handle_rfs_del_request(CHTTP_NODE *chttp_node)
{
    ...
    if(!(0 == req_body_chunk_num))
    {
        ...
        dbg_log(SEC_0056_CNGX_HTTPS, 0)(LOGSTDOUT, "error: cngx_https_handle_rfs_del_request: chunk num %ld\n", req_body_chunk_num);

        dbg_log(SEC_0056_CNGX_HTTPS, 0)(LOGSTDOUT, "error: cngx_https_handle_rfs_del_request: chunk mgr %p info\n", req_body_chunks);
        chunk_mgr_print_info(LOGSTDOUT, req_body_chunks);

        dbg_log(SEC_0056_CNGX_HTTPS, 0)(LOGSTDOUT, "error: cngx_https_handle_rfs_del_request: chunk mgr %p chars\n", req_body_chunks);
        chunk_mgr_print_chars(LOGSTDOUT, req_body_chunks);
    ...
    if(1)
    {
        ...
        if(NULL_PTR != v)
        {
            ...
            CMON_NODE_TCID(&cmon_node) = c_ipv4_to_word(v);
            dbg_log(SEC_0056_CNGX_HTTPS, 1)(LOGSTDOUT, "[DEBUG] cngx_https_handle_rfs_del_request: header rfs-tcid %s => 0x%lx\n",
                                v, CMON_NODE_TCID(&cmon_node));
        }
        ...
        if(NULL_PTR != v)
        {
            ...
            CMON_NODE_IPADDR(&cmon_node) = c_ipv4_to_word(v);
            dbg_log(SEC_0056_CNGX_HTTPS, 1)(LOGSTDOUT, "[DEBUG] cngx_https_handle_rfs_del_request: header rfs-ip %s => 0x%lx\n",
                                v, CMON_NODE_IPADDR(&cmon_node));
        }
        ...
        {
            CMON_NODE_PORT(&cmon_node) = c_str_to_word(v);
            dbg_log(SEC_0056_CNGX_HTTPS, 1)(LOGSTDOUT, "[DEBUG] cngx_https_handle_rfs_del_request: header rfs-port %s => %ld\n",
                                v, CMON_NODE_PORT(&cmon_node));
        }
        ...
        {
            CMON_NODE_MODI(&cmon_node) = c_str_to_word(v);
            dbg_log(SEC_0056_CNGX_HTTPS, 1)(LOGSTDOUT, "[DEBUG] cngx_https_handle_rfs_del_request: header rfs-modi %s => %ld\n",
                                v, CMON_NODE_MODI(&cmon_node));
        }
...
EC_BOOL cngx_https_make_rfs_del_response(CHTTP_NODE *chttp_node)
{
    if(EC_FALSE == chttp_make_response_header_common(chttp_node, (uint64_t)0))
    {
        dbg_log(SEC_0056_CNGX_HTTPS, 0)(LOGSTDOUT, "error:cngx_https_make_rfs_del_response: make response header failed\n");
        return (EC_FALSE);
    }
    ...
    if(BIT_TRUE == CHTTP_NODE_KEEPALIVE(chttp_node))
    {
        if(EC_FALSE == chttp_make_response_header_keepalive(chttp_node))
        {
            dbg_log(SEC_0056_CNGX_HTTPS, 0)(LOGSTDOUT, "error:cngx_https_make_rfs_del_response: make response header keepalive failed\n");
            return (EC_FALSE);
        }
    }
    ...
    if(EC_FALSE == chttp_make_response_header_end(chttp_node))
    {
        dbg_log(SEC_0056_CNGX_HTTPS, 0)(LOGSTDOUT, "error:cngx_https_make_rfs_del_response: make header end failed\n");
        return (EC_FALSE);
    }
...
EC_BOOL cngx_https_commit_rfs_del_response(CHTTP_NODE *chttp_node)
{
    ...
    if(NULL_PTR == csocket_cnode)
    {
        dbg_log(SEC_0056_CNGX_HTTPS, 0)(LOGSTDOUT, "error:cngx_https_commit_rfs_del_response: csocket_cnode of chttp_node %p is null\n", chttp_node);
        return (EC_FALSE);
    }
...
EC_BOOL cngx_https_commit_rfs_list_request(CHTTP_NODE *chttp_node)
{
    ...
    if(EC_FALSE == cngx_https_handle_rfs_list_request(chttp_node))
    {
        dbg_log(SEC_0056_CNGX_HTTPS, 0)(LOGSTDOUT, "error:cngx_https_commit_rfs_list_request: handle request failed\n");
        return (EC_FALSE);
    }

    if(EC_FALSE == cngx_https_make_rfs_list_response(chttp_node))
    {
        dbg_log(SEC_0056_CNGX_HTTPS, 0)(LOGSTDOUT, "error:cngx_https_commit_rfs_list_request: make response failed\n");
        return (EC_FALSE);
    }

    ret = cngx_https_commit_rfs_list_response(chttp_node);
    if(EC_FALSE == ret)
    {
        dbg_log(SEC_0056_CNGX_HTTPS, 0)(LOGSTDOUT, "error:cngx_https_commit_rfs_list_request: commit response failed\n");
        return (EC_FALSE);
    }
...
EC_BOOL cngx_https_handle_rfs_list_request(CHTTP_NODE *chttp_node)
{
    ...
    if(!(0 == req_body_chunk_num))
    {
        ...
        dbg_log(SEC_0056_CNGX_HTTPS, 0)(LOGSTDOUT, "error: cngx_https_handle_rfs_list_request: chunk num %ld\n", req_body_chunk_num);

        dbg_log(SEC_0056_CNGX_HTTPS, 0)(LOGSTDOUT, "error: cngx_https_handle_rfs_list_request: chunk mgr %p info\n", req_body_chunks);
        chunk_mgr_print_info(LOGSTDOUT, req_body_chunks);

        dbg_log(SEC_0056_CNGX_HTTPS, 0)(LOGSTDOUT, "error: cngx_https_handle_rfs_list_request: chunk mgr %p chars\n", req_body_chunks);
        chunk_mgr_print_chars(LOGSTDOUT, req_body_chunks);
...
EC_BOOL cngx_https_make_rfs_list_response(CHTTP_NODE *chttp_node)
{
    ...
    if(EC_FALSE == chttp_make_response_header_common(chttp_node, content_len))
    {
        dbg_log(SEC_0056_CNGX_HTTPS, 0)(LOGSTDOUT, "error:cngx_https_make_rfs_list_response: make response header failed\n");
        return (EC_FALSE);
    }
    ...
    if(BIT_TRUE == CHTTP_NODE_KEEPALIVE(chttp_node))
    {
        if(EC_FALSE == chttp_make_response_header_keepalive(chttp_node))
        {
            dbg_log(SEC_0056_CNGX_HTTPS, 0)(LOGSTDOUT, "error:cngx_https_make_rfs_list_response: make response header keepalive failed\n");
            return (EC_FALSE);
        }
    }
    ...
    if(EC_FALSE == chttp_make_response_header_end(chttp_node))
    {
        dbg_log(SEC_0056_CNGX_HTTPS, 0)(LOGSTDOUT, "error:cngx_https_make_rfs_list_response: make header end failed\n");
        return (EC_FALSE);
    }
    ...
    if(EC_FALSE == chttp_make_response_body_ext(chttp_node,
                                              ...
                                              (uint32_t )CBYTES_ALIGNED(content_cbytes)))
    {
        dbg_log(SEC_0056_CNGX_HTTPS, 0)(LOGSTDOUT, "error:cngx_https_make_rfs_list_response: make body with len %d failed\n",
                           (uint32_t)CBYTES_LEN(content_cbytes));
        return (EC_FALSE);
    }
    cbytes_umount(content_cbytes, NULL_PTR, NULL_PTR, NULL_PTR);
...
EC_BOOL cngx_https_commit_rfs_list_response(CHTTP_NODE *chttp_node)
{
    ...
    if(NULL_PTR == csocket_cnode)
    {
        dbg_log(SEC_0056_CNGX_HTTPS, 0)(LOGSTDOUT, "error:cngx_https_commit_rfs_list_response: csocket_cnode of chttp_node %p is null\n", chttp_node);
        return (EC_FALSE);
    }
