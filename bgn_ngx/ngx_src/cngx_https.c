...
EC_BOOL cngx_https_log_start()
{
    ...
    if(EC_TRUE == task_brd_check_is_work_tcid(TASK_BRD_TCID(task_brd)))
    {
        ...
        log_file_name = cstring_new(NULL_PTR, LOC_CNGX_0077);
    ...
    if(EC_TRUE == task_brd_check_is_work_tcid(TASK_BRD_TCID(task_brd)))
    {
        ...
        /*open log and redirect LOGUSER08 to it*/
        log_file_name = cstring_new(NULL_PTR, LOC_CNGX_0078);
...
EC_BOOL cngx_https_commit_request(CHTTP_NODE *chttp_node)
{
    ...
    if(HTTP_POST == http_parser->method)
    {
        ...
        CHTTP_NODE_LOG_TIME_WHEN_LOADED(chttp_node);/*record http request was loaded time in coroutine*/
        CHTTP_NODE_CROUTINE_NODE(chttp_node) = croutine_node;
        CROUTINE_NODE_COND_RELEASE(croutine_node, LOC_CNGX_0080);
    ...
    if(HTTP_HEAD == http_parser->method)
    {
        ...
        CHTTP_NODE_LOG_TIME_WHEN_LOADED(chttp_node);/*record http request was loaded time in coroutine*/
        CHTTP_NODE_CROUTINE_NODE(chttp_node) = croutine_node;
        CROUTINE_NODE_COND_RELEASE(croutine_node, LOC_CNGX_0081);
...
EC_BOOL cngx_https_commit_http_get(CHTTP_NODE *chttp_node)
{
    ...
    if (EC_TRUE == cngx_https_is_http_get_breathe(chttp_node))
    {
        ret = cngx_https_commit_breathe_get_request(chttp_node);
    }
    else if (EC_TRUE == cngx_https_is_http_get_logrotate(chttp_node))
    {
        ret = cngx_https_commit_logrotate_get_request(chttp_node);
    }
    else if (EC_TRUE == cngx_https_is_http_get_actsyscfg(chttp_node))
    {
    ...
    {
        ret = cngx_https_commit_xfs_up_get_request(chttp_node);
    }
    else if (SWITCH_ON == NGX_BGN_OVER_XFS_SWITCH
    && EC_TRUE == cngx_https_is_http_get_xfs_down(chttp_node))
    {
        ret = cngx_https_commit_xfs_down_get_request(chttp_node);
    }
    else if (SWITCH_ON == NGX_BGN_OVER_XFS_SWITCH
    && EC_TRUE == cngx_https_is_http_get_xfs_add(chttp_node))
    {
        ret = cngx_https_commit_xfs_add_get_request(chttp_node);
    }
    else if (SWITCH_ON == NGX_BGN_OVER_XFS_SWITCH
    && EC_TRUE == cngx_https_is_http_get_xfs_del(chttp_node))
    {
        ret = cngx_https_commit_xfs_del_get_request(chttp_node);
    }
    else if (SWITCH_ON == NGX_BGN_OVER_XFS_SWITCH
    && EC_TRUE == cngx_https_is_http_get_xfs_list(chttp_node))
    {
        ret = cngx_https_commit_xfs_list_get_request(chttp_node);
    }
    else if (SWITCH_ON == NGX_BGN_OVER_RFS_SWITCH
    && EC_TRUE == cngx_https_is_http_get_rfs_up(chttp_node))
    {
        ret = cngx_https_commit_rfs_up_get_request(chttp_node);
    }
    else if (SWITCH_ON == NGX_BGN_OVER_RFS_SWITCH
    && EC_TRUE == cngx_https_is_http_get_rfs_down(chttp_node))
    {
        ret = cngx_https_commit_rfs_down_get_request(chttp_node);
    }
    else if (SWITCH_ON == NGX_BGN_OVER_RFS_SWITCH
    && EC_TRUE == cngx_https_is_http_get_rfs_add(chttp_node))
    {
        ret = cngx_https_commit_rfs_add_get_request(chttp_node);
    }
    else if (SWITCH_ON == NGX_BGN_OVER_RFS_SWITCH
    && EC_TRUE == cngx_https_is_http_get_rfs_del(chttp_node))
    {
        ret = cngx_https_commit_rfs_del_get_request(chttp_node);
    }
    else if (SWITCH_ON == NGX_BGN_OVER_RFS_SWITCH
    && EC_TRUE == cngx_https_is_http_get_rfs_list(chttp_node))
    {
        ret = cngx_https_commit_rfs_list_get_request(chttp_node);
    }
    else
    {
        CBUFFER *uri_cbuffer;
        ...
        CHTTP_NODE_LOG_INFO_WHEN_DONE(chttp_node, "error:cngx_https_commit_http_get: invalid uri %.*s",
                            CBUFFER_USED(uri_cbuffer), CBUFFER_DATA(uri_cbuffer));
...
EC_BOOL cngx_https_handle_breathe_get_request(CHTTP_NODE *chttp_node)
{
    ...
    if(!(0 == req_body_chunk_num))
    {
        ...
        CHTTP_NODE_LOG_INFO_WHEN_DONE(chttp_node, "[DEBUG] cngx_https_handle_breathe_get_request: bad request");
    ...
    if(1)
    {
        ...
        CHTTP_NODE_LOG_INFO_WHEN_DONE(chttp_node, "[DEBUG] cngx_https_handle_breathe_get_request: memory breathing done");
...
EC_BOOL cngx_https_commit_breathe_get_response(CHTTP_NODE *chttp_node)
{
    ...
    if(NULL_PTR == csocket_cnode)
    {
        dbg_log(SEC_0056_CNGX_HTTPS, 0)(LOGSTDOUT, "error:cngx_https_commit_breathe_get_response: csocket_cnode of chttp_node %p is null\n", chttp_node);
        return (EC_FALSE);
    }
...
EC_BOOL cngx_https_handle_logrotate_get_request(CHTTP_NODE *chttp_node)
{
    ...
    if(!(0 == req_body_chunk_num))
    {
        ...
        CHTTP_NODE_LOG_TIME_WHEN_DONE(chttp_node);
        CHTTP_NODE_LOG_STAT_WHEN_DONE(chttp_node, "XFS_ERR %s %u --", "GET", CHTTP_BAD_REQUEST);
        CHTTP_NODE_LOG_INFO_WHEN_DONE(chttp_node, "[DEBUG] cngx_https_handle_logrotate_get_request: bad request");
    ...
    if(EC_TRUE == __cngx_https_uri_is_logrotate_get_op(uri_cbuffer))
    {
        ...
        if(NULL_PTR == log_index_str)
        {
            ...
            if(EC_FALSE == super_rotate_log(super_md_id, log_index))
            {
                ...
                CHTTP_NODE_LOG_INFO_WHEN_DONE(chttp_node, "error:cngx_https_handle_logrotate_get_request: log rotate %ld failed", log_index);
            ...
            CHTTP_NODE_LOG_TIME_WHEN_DONE(chttp_node);
            CHTTP_NODE_LOG_STAT_WHEN_DONE(chttp_node, "XFS_SUCC %s %u --", "GET", CHTTP_OK);
            CHTTP_NODE_LOG_INFO_WHEN_DONE(chttp_node, "[DEBUG] cngx_https_handle_logrotate_get_request: log rotate %ld done", log_index);
        ...
        {
            dbg_log(SEC_0056_CNGX_HTTPS, 0)(LOGSTDOUT, "error:cngx_https_handle_logrotate_get_request: no memory\n");

            CHTTP_NODE_LOG_TIME_WHEN_DONE(chttp_node);
            CHTTP_NODE_LOG_STAT_WHEN_DONE(chttp_node, "XFS_ERR %s %u --", "GET", CHTTP_INTERNAL_SERVER_ERROR);
            CHTTP_NODE_LOG_INFO_WHEN_DONE(chttp_node, "error:cngx_https_handle_logrotate_get_request: no memory");
        ...
        dbg_log(SEC_0056_CNGX_HTTPS, 5)(LOGSTDOUT, "[DEBUG] cngx_https_handle_logrotate_get_request: log rotate %s done\n", log_index_str);

        CHTTP_NODE_LOG_TIME_WHEN_DONE(chttp_node);
        CHTTP_NODE_LOG_STAT_WHEN_DONE(chttp_node, "XFS_SUCC %s %u --", "GET", CHTTP_OK);
        CHTTP_NODE_LOG_INFO_WHEN_DONE(chttp_node, "[DEBUG] cngx_https_handle_logrotate_get_request: log rotate %s done", log_index_str);
...
EC_BOOL cngx_https_commit_logrotate_get_response(CHTTP_NODE *chttp_node)
{
    ...
    if(NULL_PTR == csocket_cnode)
    {
        dbg_log(SEC_0056_CNGX_HTTPS, 0)(LOGSTDOUT, "error:cngx_https_commit_logrotate_get_response: csocket_cnode of chttp_node %p is null\n", chttp_node);
        return (EC_FALSE);
    }
...
EC_BOOL cngx_https_handle_actsyscfg_get_request(CHTTP_NODE *chttp_node)
{
    ...
    if(!(0 == req_body_chunk_num))
    {
        ...
        CHTTP_NODE_LOG_TIME_WHEN_DONE(chttp_node);
        CHTTP_NODE_LOG_STAT_WHEN_DONE(chttp_node, "XFS_ERR %s %u --", "GET", CHTTP_BAD_REQUEST);
        CHTTP_NODE_LOG_INFO_WHEN_DONE(chttp_node, "[DEBUG] cngx_https_handle_actsyscfg_get_request: bad request");
    ...
    if(EC_TRUE == __cngx_https_uri_is_actsyscfg_get_op(uri_cbuffer))
    {
        ...
        CHTTP_NODE_LOG_TIME_WHEN_DONE(chttp_node);
        CHTTP_NODE_LOG_STAT_WHEN_DONE(chttp_node, "XFS_SUCC %s %u --", "GET", CHTTP_OK);
        CHTTP_NODE_LOG_INFO_WHEN_DONE(chttp_node, "[DEBUG] cngx_https_handle_actsyscfg_get_request done");
...
EC_BOOL cngx_https_commit_actsyscfg_get_response(CHTTP_NODE *chttp_node)
{
    ...
    if(NULL_PTR == csocket_cnode)
    {
        dbg_log(SEC_0056_CNGX_HTTPS, 0)(LOGSTDOUT, "error:cngx_https_commit_actsyscfg_get_response: csocket_cnode of chttp_node %p is null\n", chttp_node);
        return (EC_FALSE);
    }
...
EC_BOOL cngx_https_handle_xfs_up_get_request(CHTTP_NODE *chttp_node)
{
    ...
    if(!(0 == req_body_chunk_num))
    {
        ...
        CHTTP_NODE_LOG_TIME_WHEN_DONE(chttp_node);
        CHTTP_NODE_LOG_STAT_WHEN_DONE(chttp_node, "XFS_ERR %s %u --", "GET", CHTTP_BAD_REQUEST);
        CHTTP_NODE_LOG_INFO_WHEN_DONE(chttp_node, "[DEBUG] cngx_https_handle_xfs_up_get_request");
    ...
    if(EC_TRUE == __cngx_https_uri_is_xfs_up_get_op(uri_cbuffer))
    {
        ...
        if(CMPI_ERROR_MODI == cmon_id)
        {
            ...
            CHTTP_NODE_LOG_INFO_WHEN_DONE(chttp_node, "error:cngx_https_handle_xfs_up_get_request: no cmon start");
        ...
        if(NULL_PTR != v)
        {
            if(EC_FALSE == c_ipv4_is_ok(v))
            {
                ...
                CHTTP_NODE_LOG_INFO_WHEN_DONE(chttp_node, "error:cngx_https_handle_xfs_up_get_request: invalid xfs-tcid '%s'", v);
        ...
        if(NULL_PTR != v)
        {
            if(EC_FALSE == c_ipv4_is_ok(v))
            {
                ...
                CHTTP_NODE_LOG_TIME_WHEN_DONE(chttp_node);
                CHTTP_NODE_LOG_STAT_WHEN_DONE(chttp_node, "XFSMON_FAIL %s %u --", "GET", CHTTP_BAD_REQUEST);
                CHTTP_NODE_LOG_INFO_WHEN_DONE(chttp_node, "error:cngx_https_handle_xfs_up_get_request: invalid xfs-ip '%s'", v);
        ...
        if(EC_FALSE == cmon_set_node_up(cmon_id, &cmon_node))
        {
            ...
            CHTTP_NODE_LOG_INFO_WHEN_DONE(chttp_node, "error:cngx_https_handle_xfs_up_get_request: set up xfs %s failed", c_word_to_ipv4(CMON_NODE_TCID(&cmon_node)));
        ...
        CHTTP_NODE_LOG_INFO_WHEN_DONE(chttp_node, "[DEBUG] cngx_https_handle_xfs_up_get_request: set up xfs %s done", c_word_to_ipv4(CMON_NODE_TCID(&cmon_node)));
...
EC_BOOL cngx_https_commit_xfs_up_get_response(CHTTP_NODE *chttp_node)
{
    ...
    if(NULL_PTR == csocket_cnode)
    {
        dbg_log(SEC_0056_CNGX_HTTPS, 0)(LOGSTDOUT, "error:cngx_https_commit_xfs_up_get_response: csocket_cnode of chttp_node %p is null\n", chttp_node);
        return (EC_FALSE);
    }
...
EC_BOOL cngx_https_handle_xfs_down_get_request(CHTTP_NODE *chttp_node)
{
    ...
    if(!(0 == req_body_chunk_num))
    {
        ...
        CHTTP_NODE_LOG_TIME_WHEN_DONE(chttp_node);
        CHTTP_NODE_LOG_STAT_WHEN_DONE(chttp_node, "XFS_ERR %s %u --", "GET", CHTTP_BAD_REQUEST);
        CHTTP_NODE_LOG_INFO_WHEN_DONE(chttp_node, "[DEBUG] cngx_https_handle_xfs_down_get_request");
    ...
    if(EC_TRUE == __cngx_https_uri_is_xfs_down_get_op(uri_cbuffer))
    {
        ...
        if(CMPI_ERROR_MODI == cmon_id)
        {
            ...
            CHTTP_NODE_LOG_TIME_WHEN_DONE(chttp_node);
            CHTTP_NODE_LOG_STAT_WHEN_DONE(chttp_node, "XFSMON_FAIL %s %u --", "GET", CHTTP_NOT_IMPLEMENTED);
            CHTTP_NODE_LOG_INFO_WHEN_DONE(chttp_node, "error:cngx_https_handle_xfs_down_get_request: no cmon start");
        ...
        if(NULL_PTR != v)
        {
            if(EC_FALSE == c_ipv4_is_ok(v))
            {
                ...
                CHTTP_NODE_LOG_TIME_WHEN_DONE(chttp_node);
                CHTTP_NODE_LOG_STAT_WHEN_DONE(chttp_node, "XFSMON_FAIL %s %u --", "GET", CHTTP_BAD_REQUEST);
                CHTTP_NODE_LOG_INFO_WHEN_DONE(chttp_node, "error:cngx_https_handle_xfs_down_get_request: invalid xfs-tcid '%s'", v);
        ...
        if(NULL_PTR != v)
        {
            if(EC_FALSE == c_ipv4_is_ok(v))
            {
                ...
                CHTTP_NODE_LOG_TIME_WHEN_DONE(chttp_node);
                CHTTP_NODE_LOG_STAT_WHEN_DONE(chttp_node, "XFSMON_FAIL %s %u --", "GET", CHTTP_BAD_REQUEST);
                CHTTP_NODE_LOG_INFO_WHEN_DONE(chttp_node, "error:cngx_https_handle_xfs_down_get_request: invalid xfs-ip '%s'", v);
        ...
        if(EC_FALSE == cmon_set_node_down(cmon_id, &cmon_node))
        {
            ...
            CHTTP_NODE_LOG_TIME_WHEN_DONE(chttp_node);
            CHTTP_NODE_LOG_STAT_WHEN_DONE(chttp_node, "XFSMON_FAIL %s %u --", "GET", CHTTP_FORBIDDEN);
            CHTTP_NODE_LOG_INFO_WHEN_DONE(chttp_node, "error:cngx_https_handle_xfs_down_get_request: set down xfs %s failed", c_word_to_ipv4(CMON_NODE_TCID(&cmon_node)));
        ...
        CHTTP_NODE_LOG_TIME_WHEN_DONE(chttp_node);
        CHTTP_NODE_LOG_STAT_WHEN_DONE(chttp_node, "XFSMON_SUCC %s %u --", "GET", CHTTP_OK);
        CHTTP_NODE_LOG_INFO_WHEN_DONE(chttp_node, "[DEBUG] cngx_https_handle_xfs_down_get_request: set down xfs %s done", c_word_to_ipv4(CMON_NODE_TCID(&cmon_node)));
...
EC_BOOL cngx_https_commit_xfs_down_get_response(CHTTP_NODE *chttp_node)
{
    ...
    if(NULL_PTR == csocket_cnode)
    {
        dbg_log(SEC_0056_CNGX_HTTPS, 0)(LOGSTDOUT, "error:cngx_https_commit_xfs_down_get_response: csocket_cnode of chttp_node %p is null\n", chttp_node);
        return (EC_FALSE);
    }
...
EC_BOOL cngx_https_handle_xfs_add_get_request(CHTTP_NODE *chttp_node)
{
    ...
    if(!(0 == req_body_chunk_num))
    {
        ...
        CHTTP_NODE_LOG_TIME_WHEN_DONE(chttp_node);
        CHTTP_NODE_LOG_STAT_WHEN_DONE(chttp_node, "XFS_ERR %s %u --", "GET", CHTTP_BAD_REQUEST);
        CHTTP_NODE_LOG_INFO_WHEN_DONE(chttp_node, "[DEBUG] cngx_https_handle_xfs_add_get_request");
    ...
    if(EC_TRUE == __cngx_https_uri_is_xfs_add_get_op(uri_cbuffer))
    {
        ...
        if(CMPI_ERROR_MODI == cmon_id)
        {
            ...
            CHTTP_NODE_LOG_TIME_WHEN_DONE(chttp_node);
            CHTTP_NODE_LOG_STAT_WHEN_DONE(chttp_node, "XFSMON_FAIL %s %u --", "GET", CHTTP_NOT_IMPLEMENTED);
            CHTTP_NODE_LOG_INFO_WHEN_DONE(chttp_node, "error:cngx_https_handle_xfs_add_get_request: no cmon start");
        ...
        if(NULL_PTR != v)
        {
            if(EC_FALSE == c_ipv4_is_ok(v))
            {
                ...
                CHTTP_NODE_LOG_TIME_WHEN_DONE(chttp_node);
                CHTTP_NODE_LOG_STAT_WHEN_DONE(chttp_node, "XFSMON_FAIL %s %u --", "GET", CHTTP_BAD_REQUEST);
                CHTTP_NODE_LOG_INFO_WHEN_DONE(chttp_node, "error:cngx_https_handle_xfs_add_get_request: invalid xfs-tcid '%s'", v);
        ...
        if(NULL_PTR != v)
        {
            if(EC_FALSE == c_ipv4_is_ok(v))
            {
                ...
                CHTTP_NODE_LOG_TIME_WHEN_DONE(chttp_node);
                CHTTP_NODE_LOG_STAT_WHEN_DONE(chttp_node, "XFSMON_FAIL %s %u --", "GET", CHTTP_BAD_REQUEST);
                CHTTP_NODE_LOG_INFO_WHEN_DONE(chttp_node, "error:cngx_https_handle_xfs_add_get_request: invalid xfs-ip '%s'", v);
        ...
        if(EC_FALSE == cmon_add_node(cmon_id, &cmon_node))
        {
            ...
            CHTTP_NODE_LOG_TIME_WHEN_DONE(chttp_node);
            CHTTP_NODE_LOG_STAT_WHEN_DONE(chttp_node, "XFSMON_FAIL %s %u --", "GET", CHTTP_FORBIDDEN);
            CHTTP_NODE_LOG_INFO_WHEN_DONE(chttp_node, "error:cngx_https_handle_xfs_add_get_request: add xfs %s failed", c_word_to_ipv4(CMON_NODE_TCID(&cmon_node)));
        ...
        CHTTP_NODE_LOG_TIME_WHEN_DONE(chttp_node);
        CHTTP_NODE_LOG_STAT_WHEN_DONE(chttp_node, "XFSMON_SUCC %s %u --", "GET", CHTTP_OK);
        CHTTP_NODE_LOG_INFO_WHEN_DONE(chttp_node, "[DEBUG] cngx_https_handle_xfs_add_get_request: add xfs %s done", c_word_to_ipv4(CMON_NODE_TCID(&cmon_node)));
...
EC_BOOL cngx_https_commit_xfs_add_get_response(CHTTP_NODE *chttp_node)
{
    ...
    if(NULL_PTR == csocket_cnode)
    {
        dbg_log(SEC_0056_CNGX_HTTPS, 0)(LOGSTDOUT, "error:cngx_https_commit_xfs_add_get_response: csocket_cnode of chttp_node %p is null\n", chttp_node);
        return (EC_FALSE);
    }
...
EC_BOOL cngx_https_handle_xfs_del_get_request(CHTTP_NODE *chttp_node)
{
    ...
    if(!(0 == req_body_chunk_num))
    {
        ...
        CHTTP_NODE_LOG_TIME_WHEN_DONE(chttp_node);
        CHTTP_NODE_LOG_STAT_WHEN_DONE(chttp_node, "XFS_ERR %s %u --", "GET", CHTTP_BAD_REQUEST);
        CHTTP_NODE_LOG_INFO_WHEN_DONE(chttp_node, "[DEBUG] cngx_https_handle_xfs_del_get_request");
    ...
    if(EC_TRUE == __cngx_https_uri_is_xfs_del_get_op(uri_cbuffer))
    {
        ...
        if(CMPI_ERROR_MODI == cmon_id)
        {
            ...
            CHTTP_NODE_LOG_TIME_WHEN_DONE(chttp_node);
            CHTTP_NODE_LOG_STAT_WHEN_DONE(chttp_node, "XFSMON_FAIL %s %u --", "GET", CHTTP_NOT_IMPLEMENTED);
            CHTTP_NODE_LOG_INFO_WHEN_DONE(chttp_node, "error:cngx_https_handle_xfs_del_get_request: no cmon start");
        ...
        if(NULL_PTR != v)
        {
            if(EC_FALSE == c_ipv4_is_ok(v))
            {
                ...
                CHTTP_NODE_LOG_TIME_WHEN_DONE(chttp_node);
                CHTTP_NODE_LOG_STAT_WHEN_DONE(chttp_node, "XFSMON_FAIL %s %u --", "GET", CHTTP_BAD_REQUEST);
                CHTTP_NODE_LOG_INFO_WHEN_DONE(chttp_node, "error:cngx_https_handle_xfs_del_get_request: invalid xfs-tcid '%s'", v);
        ...
        if(NULL_PTR != v)
        {
            if(EC_FALSE == c_ipv4_is_ok(v))
            {
                ...
                CHTTP_NODE_LOG_TIME_WHEN_DONE(chttp_node);
                CHTTP_NODE_LOG_STAT_WHEN_DONE(chttp_node, "XFSMON_FAIL %s %u --", "GET", CHTTP_BAD_REQUEST);
                CHTTP_NODE_LOG_INFO_WHEN_DONE(chttp_node, "error:cngx_https_handle_xfs_del_get_request: invalid xfs-ip '%s'", v);
        ...
        if(EC_FALSE == cmon_del_node(cmon_id, &cmon_node))
        {
            ...
            CHTTP_NODE_LOG_TIME_WHEN_DONE(chttp_node);
            CHTTP_NODE_LOG_STAT_WHEN_DONE(chttp_node, "XFSMON_FAIL %s %u --", "GET", CHTTP_FORBIDDEN);
            CHTTP_NODE_LOG_INFO_WHEN_DONE(chttp_node, "error:cngx_https_handle_xfs_del_get_request: del xfs %s failed", c_word_to_ipv4(CMON_NODE_TCID(&cmon_node)));
        ...
        CHTTP_NODE_LOG_TIME_WHEN_DONE(chttp_node);
        CHTTP_NODE_LOG_STAT_WHEN_DONE(chttp_node, "XFSMON_SUCC %s %u --", "GET", CHTTP_OK);
        CHTTP_NODE_LOG_INFO_WHEN_DONE(chttp_node, "[DEBUG] cngx_https_handle_xfs_del_get_request: del xfs %s done", c_word_to_ipv4(CMON_NODE_TCID(&cmon_node)));
...
EC_BOOL cngx_https_commit_xfs_del_get_response(CHTTP_NODE *chttp_node)
{
    ...
    if(NULL_PTR == csocket_cnode)
    {
        dbg_log(SEC_0056_CNGX_HTTPS, 0)(LOGSTDOUT, "error:cngx_https_commit_xfs_del_get_response: csocket_cnode of chttp_node %p is null\n", chttp_node);
        return (EC_FALSE);
    }
...
EC_BOOL cngx_https_handle_xfs_list_get_request(CHTTP_NODE *chttp_node)
{
    ...
    if(!(0 == req_body_chunk_num))
    {
        ...
        CHTTP_NODE_LOG_TIME_WHEN_DONE(chttp_node);
        CHTTP_NODE_LOG_STAT_WHEN_DONE(chttp_node, "XFS_ERR %s %u --", "GET", CHTTP_BAD_REQUEST);
        CHTTP_NODE_LOG_INFO_WHEN_DONE(chttp_node, "[DEBUG] cngx_https_handle_xfs_list_get_request");
    ...
    if(EC_TRUE == __cngx_https_uri_is_xfs_list_get_op(uri_cbuffer))
    {
        ...
        if(CMPI_ERROR_MODI == cmon_id)
        {
            ...
            CHTTP_NODE_LOG_TIME_WHEN_DONE(chttp_node);
            CHTTP_NODE_LOG_STAT_WHEN_DONE(chttp_node, "XFSMON_FAIL %s %u --", "GET", CHTTP_NOT_IMPLEMENTED);
            CHTTP_NODE_LOG_INFO_WHEN_DONE(chttp_node, "error:cngx_https_handle_xfs_list_get_request: no cmon start");
        ...
        CHTTP_NODE_LOG_TIME_WHEN_DONE(chttp_node);
        CHTTP_NODE_LOG_STAT_WHEN_DONE(chttp_node, "XFSMON_SUCC %s %u --", "GET", CHTTP_OK);
        CHTTP_NODE_LOG_INFO_WHEN_DONE(chttp_node, "[DEBUG] cngx_https_handle_xfs_list_get_request: list xfs done");
...
EC_BOOL cngx_https_commit_xfs_list_get_response(CHTTP_NODE *chttp_node)
{
    ...
    if(NULL_PTR == csocket_cnode)
    {
        dbg_log(SEC_0056_CNGX_HTTPS, 0)(LOGSTDOUT, "error:cngx_https_commit_xfs_list_get_response: csocket_cnode of chttp_node %p is null\n", chttp_node);
        return (EC_FALSE);
    }
...
EC_BOOL cngx_https_handle_rfs_up_get_request(CHTTP_NODE *chttp_node)
{
    ...
    if(!(0 == req_body_chunk_num))
    {
        ...
        CHTTP_NODE_LOG_INFO_WHEN_DONE(chttp_node, "[DEBUG] cngx_https_handle_rfs_up_get_request");
    ...
    if(EC_TRUE == __cngx_https_uri_is_rfs_up_get_op(uri_cbuffer))
    {
        ...
        if(CMPI_ERROR_MODI == cmon_id)
        {
            ...
            CHTTP_NODE_LOG_INFO_WHEN_DONE(chttp_node, "error:cngx_https_handle_rfs_up_get_request: no cmon start");
        ...
        if(NULL_PTR != v)
        {
            if(EC_FALSE == c_ipv4_is_ok(v))
            {
                ...
                CHTTP_NODE_LOG_INFO_WHEN_DONE(chttp_node, "error:cngx_https_handle_rfs_up_get_request: invalid rfs-tcid '%s'", v);
        ...
        if(NULL_PTR != v)
        {
            if(EC_FALSE == c_ipv4_is_ok(v))
            {
                ...
                CHTTP_NODE_LOG_TIME_WHEN_DONE(chttp_node);
                CHTTP_NODE_LOG_STAT_WHEN_DONE(chttp_node, "RFSMON_FAIL %s %u --", "GET", CHTTP_BAD_REQUEST);
                CHTTP_NODE_LOG_INFO_WHEN_DONE(chttp_node, "error:cngx_https_handle_rfs_up_get_request: invalid rfs-ip '%s'", v);
        ...
        if(EC_FALSE == cmon_set_node_up(cmon_id, &cmon_node))
        {
            ...
            CHTTP_NODE_LOG_INFO_WHEN_DONE(chttp_node, "error:cngx_https_handle_rfs_up_get_request: set up rfs %s failed", c_word_to_ipv4(CMON_NODE_TCID(&cmon_node)));
        ...
        CHTTP_NODE_LOG_INFO_WHEN_DONE(chttp_node, "[DEBUG] cngx_https_handle_rfs_up_get_request: set up rfs %s done", c_word_to_ipv4(CMON_NODE_TCID(&cmon_node)));
...
EC_BOOL cngx_https_commit_rfs_up_get_response(CHTTP_NODE *chttp_node)
{
    ...
    if(NULL_PTR == csocket_cnode)
    {
        dbg_log(SEC_0056_CNGX_HTTPS, 0)(LOGSTDOUT, "error:cngx_https_commit_rfs_up_get_response: csocket_cnode of chttp_node %p is null\n", chttp_node);
        return (EC_FALSE);
    }
...
EC_BOOL cngx_https_handle_rfs_down_get_request(CHTTP_NODE *chttp_node)
{
    ...
    if(!(0 == req_body_chunk_num))
    {
        ...
        CHTTP_NODE_LOG_TIME_WHEN_DONE(chttp_node);
        CHTTP_NODE_LOG_STAT_WHEN_DONE(chttp_node, "RFS_ERR %s %u --", "GET", CHTTP_BAD_REQUEST);
        CHTTP_NODE_LOG_INFO_WHEN_DONE(chttp_node, "[DEBUG] cngx_https_handle_rfs_down_get_request");
    ...
    if(EC_TRUE == __cngx_https_uri_is_rfs_down_get_op(uri_cbuffer))
    {
        ...
        if(CMPI_ERROR_MODI == cmon_id)
        {
            ...
            CHTTP_NODE_LOG_TIME_WHEN_DONE(chttp_node);
            CHTTP_NODE_LOG_STAT_WHEN_DONE(chttp_node, "RFSMON_FAIL %s %u --", "GET", CHTTP_NOT_IMPLEMENTED);
            CHTTP_NODE_LOG_INFO_WHEN_DONE(chttp_node, "error:cngx_https_handle_rfs_down_get_request: no cmon start");
        ...
        if(NULL_PTR != v)
        {
            if(EC_FALSE == c_ipv4_is_ok(v))
            {
                ...
                CHTTP_NODE_LOG_TIME_WHEN_DONE(chttp_node);
                CHTTP_NODE_LOG_STAT_WHEN_DONE(chttp_node, "RFSMON_FAIL %s %u --", "GET", CHTTP_BAD_REQUEST);
                CHTTP_NODE_LOG_INFO_WHEN_DONE(chttp_node, "error:cngx_https_handle_rfs_down_get_request: invalid rfs-tcid '%s'", v);
        ...
        if(NULL_PTR != v)
        {
            if(EC_FALSE == c_ipv4_is_ok(v))
            {
                ...
                CHTTP_NODE_LOG_TIME_WHEN_DONE(chttp_node);
                CHTTP_NODE_LOG_STAT_WHEN_DONE(chttp_node, "RFSMON_FAIL %s %u --", "GET", CHTTP_BAD_REQUEST);
                CHTTP_NODE_LOG_INFO_WHEN_DONE(chttp_node, "error:cngx_https_handle_rfs_down_get_request: invalid rfs-ip '%s'", v);
        ...
        if(EC_FALSE == cmon_set_node_down(cmon_id, &cmon_node))
        {
            ...
            CHTTP_NODE_LOG_TIME_WHEN_DONE(chttp_node);
            CHTTP_NODE_LOG_STAT_WHEN_DONE(chttp_node, "RFSMON_FAIL %s %u --", "GET", CHTTP_FORBIDDEN);
            CHTTP_NODE_LOG_INFO_WHEN_DONE(chttp_node, "error:cngx_https_handle_rfs_down_get_request: set down rfs %s failed", c_word_to_ipv4(CMON_NODE_TCID(&cmon_node)));
        ...
        CHTTP_NODE_LOG_TIME_WHEN_DONE(chttp_node);
        CHTTP_NODE_LOG_STAT_WHEN_DONE(chttp_node, "RFSMON_SUCC %s %u --", "GET", CHTTP_OK);
        CHTTP_NODE_LOG_INFO_WHEN_DONE(chttp_node, "[DEBUG] cngx_https_handle_rfs_down_get_request: set down rfs %s done", c_word_to_ipv4(CMON_NODE_TCID(&cmon_node)));
...
EC_BOOL cngx_https_commit_rfs_down_get_response(CHTTP_NODE *chttp_node)
{
    ...
    if(NULL_PTR == csocket_cnode)
    {
        dbg_log(SEC_0056_CNGX_HTTPS, 0)(LOGSTDOUT, "error:cngx_https_commit_rfs_down_get_response: csocket_cnode of chttp_node %p is null\n", chttp_node);
        return (EC_FALSE);
    }
...
EC_BOOL cngx_https_handle_rfs_add_get_request(CHTTP_NODE *chttp_node)
{
    ...
    if(!(0 == req_body_chunk_num))
    {
        ...
        CHTTP_NODE_LOG_TIME_WHEN_DONE(chttp_node);
        CHTTP_NODE_LOG_STAT_WHEN_DONE(chttp_node, "RFS_ERR %s %u --", "GET", CHTTP_BAD_REQUEST);
        CHTTP_NODE_LOG_INFO_WHEN_DONE(chttp_node, "[DEBUG] cngx_https_handle_rfs_add_get_request");
    ...
    if(EC_TRUE == __cngx_https_uri_is_rfs_add_get_op(uri_cbuffer))
    {
        ...
        if(CMPI_ERROR_MODI == cmon_id)
        {
            ...
            CHTTP_NODE_LOG_TIME_WHEN_DONE(chttp_node);
            CHTTP_NODE_LOG_STAT_WHEN_DONE(chttp_node, "RFSMON_FAIL %s %u --", "GET", CHTTP_NOT_IMPLEMENTED);
            CHTTP_NODE_LOG_INFO_WHEN_DONE(chttp_node, "error:cngx_https_handle_rfs_add_get_request: no cmon start");
        ...
        if(NULL_PTR != v)
        {
            if(EC_FALSE == c_ipv4_is_ok(v))
            {
                ...
                CHTTP_NODE_LOG_TIME_WHEN_DONE(chttp_node);
                CHTTP_NODE_LOG_STAT_WHEN_DONE(chttp_node, "RFSMON_FAIL %s %u --", "GET", CHTTP_BAD_REQUEST);
                CHTTP_NODE_LOG_INFO_WHEN_DONE(chttp_node, "error:cngx_https_handle_rfs_add_get_request: invalid rfs-tcid '%s'", v);
        ...
        if(NULL_PTR != v)
        {
            if(EC_FALSE == c_ipv4_is_ok(v))
            {
                ...
                CHTTP_NODE_LOG_TIME_WHEN_DONE(chttp_node);
                CHTTP_NODE_LOG_STAT_WHEN_DONE(chttp_node, "RFSMON_FAIL %s %u --", "GET", CHTTP_BAD_REQUEST);
                CHTTP_NODE_LOG_INFO_WHEN_DONE(chttp_node, "error:cngx_https_handle_rfs_add_get_request: invalid rfs-ip '%s'", v);
        ...
        if(EC_FALSE == cmon_add_node(cmon_id, &cmon_node))
        {
            ...
            CHTTP_NODE_LOG_TIME_WHEN_DONE(chttp_node);
            CHTTP_NODE_LOG_STAT_WHEN_DONE(chttp_node, "RFSMON_FAIL %s %u --", "GET", CHTTP_FORBIDDEN);
            CHTTP_NODE_LOG_INFO_WHEN_DONE(chttp_node, "error:cngx_https_handle_rfs_add_get_request: add rfs %s failed", c_word_to_ipv4(CMON_NODE_TCID(&cmon_node)));
        ...
        CHTTP_NODE_LOG_TIME_WHEN_DONE(chttp_node);
        CHTTP_NODE_LOG_STAT_WHEN_DONE(chttp_node, "RFSMON_SUCC %s %u --", "GET", CHTTP_OK);
        CHTTP_NODE_LOG_INFO_WHEN_DONE(chttp_node, "[DEBUG] cngx_https_handle_rfs_add_get_request: add rfs %s done", c_word_to_ipv4(CMON_NODE_TCID(&cmon_node)));
...
EC_BOOL cngx_https_commit_rfs_add_get_response(CHTTP_NODE *chttp_node)
{
    ...
    if(NULL_PTR == csocket_cnode)
    {
        dbg_log(SEC_0056_CNGX_HTTPS, 0)(LOGSTDOUT, "error:cngx_https_commit_rfs_add_get_response: csocket_cnode of chttp_node %p is null\n", chttp_node);
        return (EC_FALSE);
    }
...
EC_BOOL cngx_https_handle_rfs_del_get_request(CHTTP_NODE *chttp_node)
{
    ...
    if(!(0 == req_body_chunk_num))
    {
        ...
        CHTTP_NODE_LOG_TIME_WHEN_DONE(chttp_node);
        CHTTP_NODE_LOG_STAT_WHEN_DONE(chttp_node, "RFS_ERR %s %u --", "GET", CHTTP_BAD_REQUEST);
        CHTTP_NODE_LOG_INFO_WHEN_DONE(chttp_node, "[DEBUG] cngx_https_handle_rfs_del_get_request");
    ...
    if(EC_TRUE == __cngx_https_uri_is_rfs_del_get_op(uri_cbuffer))
    {
        ...
        if(CMPI_ERROR_MODI == cmon_id)
        {
            ...
            CHTTP_NODE_LOG_TIME_WHEN_DONE(chttp_node);
            CHTTP_NODE_LOG_STAT_WHEN_DONE(chttp_node, "RFSMON_FAIL %s %u --", "GET", CHTTP_NOT_IMPLEMENTED);
            CHTTP_NODE_LOG_INFO_WHEN_DONE(chttp_node, "error:cngx_https_handle_rfs_del_get_request: no cmon start");
        ...
        if(NULL_PTR != v)
        {
            if(EC_FALSE == c_ipv4_is_ok(v))
            {
                ...
                CHTTP_NODE_LOG_TIME_WHEN_DONE(chttp_node);
                CHTTP_NODE_LOG_STAT_WHEN_DONE(chttp_node, "RFSMON_FAIL %s %u --", "GET", CHTTP_BAD_REQUEST);
                CHTTP_NODE_LOG_INFO_WHEN_DONE(chttp_node, "error:cngx_https_handle_rfs_del_get_request: invalid rfs-tcid '%s'", v);
        ...
        if(NULL_PTR != v)
        {
            if(EC_FALSE == c_ipv4_is_ok(v))
            {
                ...
                CHTTP_NODE_LOG_TIME_WHEN_DONE(chttp_node);
                CHTTP_NODE_LOG_STAT_WHEN_DONE(chttp_node, "RFSMON_FAIL %s %u --", "GET", CHTTP_BAD_REQUEST);
                CHTTP_NODE_LOG_INFO_WHEN_DONE(chttp_node, "error:cngx_https_handle_rfs_del_get_request: invalid rfs-ip '%s'", v);
        ...
        if(EC_FALSE == cmon_del_node(cmon_id, &cmon_node))
        {
            ...
            CHTTP_NODE_LOG_TIME_WHEN_DONE(chttp_node);
            CHTTP_NODE_LOG_STAT_WHEN_DONE(chttp_node, "RFSMON_FAIL %s %u --", "GET", CHTTP_FORBIDDEN);
            CHTTP_NODE_LOG_INFO_WHEN_DONE(chttp_node, "error:cngx_https_handle_rfs_del_get_request: del rfs %s failed", c_word_to_ipv4(CMON_NODE_TCID(&cmon_node)));
        ...
        CHTTP_NODE_LOG_TIME_WHEN_DONE(chttp_node);
        CHTTP_NODE_LOG_STAT_WHEN_DONE(chttp_node, "RFSMON_SUCC %s %u --", "GET", CHTTP_OK);
        CHTTP_NODE_LOG_INFO_WHEN_DONE(chttp_node, "[DEBUG] cngx_https_handle_rfs_del_get_request: del rfs %s done", c_word_to_ipv4(CMON_NODE_TCID(&cmon_node)));
...
EC_BOOL cngx_https_commit_rfs_del_get_response(CHTTP_NODE *chttp_node)
{
    ...
    if(NULL_PTR == csocket_cnode)
    {
        dbg_log(SEC_0056_CNGX_HTTPS, 0)(LOGSTDOUT, "error:cngx_https_commit_rfs_del_get_response: csocket_cnode of chttp_node %p is null\n", chttp_node);
        return (EC_FALSE);
    }
...
EC_BOOL cngx_https_handle_rfs_list_get_request(CHTTP_NODE *chttp_node)
{
    ...
    if(!(0 == req_body_chunk_num))
    {
        ...
        CHTTP_NODE_LOG_TIME_WHEN_DONE(chttp_node);
        CHTTP_NODE_LOG_STAT_WHEN_DONE(chttp_node, "RFS_ERR %s %u --", "GET", CHTTP_BAD_REQUEST);
        CHTTP_NODE_LOG_INFO_WHEN_DONE(chttp_node, "[DEBUG] cngx_https_handle_rfs_list_get_request");
    ...
    if(EC_TRUE == __cngx_https_uri_is_rfs_list_get_op(uri_cbuffer))
    {
        ...
        if(CMPI_ERROR_MODI == cmon_id)
        {
            ...
            CHTTP_NODE_LOG_TIME_WHEN_DONE(chttp_node);
            CHTTP_NODE_LOG_STAT_WHEN_DONE(chttp_node, "RFSMON_FAIL %s %u --", "GET", CHTTP_NOT_IMPLEMENTED);
            CHTTP_NODE_LOG_INFO_WHEN_DONE(chttp_node, "error:cngx_https_handle_rfs_list_get_request: no cmon start");
        ...
        CHTTP_NODE_LOG_TIME_WHEN_DONE(chttp_node);
        CHTTP_NODE_LOG_STAT_WHEN_DONE(chttp_node, "RFSMON_SUCC %s %u --", "GET", CHTTP_OK);
        CHTTP_NODE_LOG_INFO_WHEN_DONE(chttp_node, "[DEBUG] cngx_https_handle_rfs_list_get_request: list rfs done");
