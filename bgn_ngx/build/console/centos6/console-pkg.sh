...
#rpmbuild -bb ${fs_type}.spec

#rpmbuild -ba ${fs_type}.spec || exit 1
...
# prepare makefile
rm -f ${CUR_DIR}/${fs_type}.spec
cp -af ${pkg_src}/bin/${fs_type}.spec ${CUR_DIR}/${fs_type}.spec

mkdir -vp $SOURCES/${fs_type}-${version}
cp -af ${CUR_DIR}/${pkg_src}/*      $SOURCES/${fs_type}-${version}
cp -af ${CUR_DIR}/${fs_type}.spec   $SOURCES/

# replace files
cp -af ${CUR_DIR}/${pkg_src}/bin/none.xml $SOURCES/${fs_type}-${version}/bin/config.xml
mkdir -vp $BUILDROOT/${DEPEND_DIR}
cp -af ${DEPEND_DIR}/* $BUILDROOT/${DEPEND_DIR}  || exit 1

sed -i "s@^Version:.*@Version: $version@" $SOURCES/${fs_type}.spec

cd $SOURCES && tar zcvf ${fs_type}-${version}.tar.gz ${fs_type}-${version}

QA_RPATHS=0x0013 rpmbuild -bb ${fs_type}.spec && mv $BUILD_DIR/RPMS/x86_64/*.rpm $CUR_DIR

# clean up
rm -rf ${BUILD_DIR}
rm -rf ${CUR_DIR}/${fs_type}.spec
