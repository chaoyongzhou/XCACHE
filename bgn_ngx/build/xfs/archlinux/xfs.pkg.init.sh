...
g_32G_k=33554432
g_disk_min_size=34359738368 # min size of all disk available space. here initialize it as invalid max value
g_mem_cache_size=0
...
gen_hsxfs_one_node()
{
    ...
    echo_debug "[DEBUG] cmd: '${dbg_exe} \"${dbg_params}\" \"${dbg_params}\" \"${tcid}\" \"${g_xfs_bin_dir}/config.xml\" &'"
...
gen_hsxfs_one_service()
{
    ...
    service_template=${g_xfs_service_dir}/xfs.pkg.service
    ...
    sed -i "s@\${xfs_path}@${cache_dir}@g" ${service_file}
...
count_disk_min_size()
{
	...
	echo_debug "[DEBUG] count_disk_min_size: ${cache_dir} => \"${disk_size_str}\""
        ...
        disk_size=$(expr ${disk_size_str} / 1024)
	echo_debug "[DEBUG] count_disk_min_size: ${cache_dir} => ${disk_size_str} bytes => ${disk_size} KB"
...
gen_hsxfs_all_services()
{
    ...
    do
        ...
        gen_hsxfs_one_service ${disk_idx} ${cache_dir}
        if [ $? != 0 ]; then
