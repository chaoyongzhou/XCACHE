...
gdbReadHeader(GDatabase *db)
{
    ...
    if (rawFileRead(db->idxRawFile, 0, buffer, DB_HEADER_DATA_SIZE, 1, LOC_DB_0056) != 1)
    {
    ...
    if (0 != __safe_strncmp(buffer + DB_OFFSET_MAGIC, (const uint8_t *)DB_MAGIC, 8, LOC_DB_0057))
    {
...
gdbWriteHeader(GDatabase *db)
{
    ...
    rawFileWrite(db->idxRawFile, offset, version, sizeof(uint8_t), 2, LOC_DB_0058);
    offset += (sizeof(uint8_t) * 2);

    rawFileWrite(db->idxRawFile, offset, &type,   sizeof(uint8_t), 1, LOC_DB_0059);
