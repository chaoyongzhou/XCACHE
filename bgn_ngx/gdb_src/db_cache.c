...
gdbCacheAddBlockNoLock(GDatabase *db, GdbBlock *block)
{
    ...
    if (db->openBlockCount >= db->openBlockSize)
    {
        ...
        MEM_CHECK(newBlocks = (GdbBlock **)SAFE_MALLOC(newSize * sizeof(GdbBlock *), LOC_DB_0115));
        ...
        SAFE_FREE(db->openBlocks, LOC_DB_0116);
...
gdbCacheAddBlock(GDatabase *db, GdbBlock *block)
{
    ...
    gdbLockFreeBlockList(db, DB_WRITE_LOCK, LOC_DB_0117);
    ...
    gdbUnlockFreeBlockList(db, LOC_DB_0118);
    return block_cached;
}
...
gdbCacheRemoveBlock(GDatabase *db, GdbBlock *block)
{
    ...
    gdbLockFreeBlockList(db, DB_WRITE_LOCK, LOC_DB_0119);
    refCount = gdbCacheRemoveBlockNoLock(db, block);
    gdbUnlockFreeBlockList(db, LOC_DB_0120);
    return refCount;
}
...
gdbCacheGetBlock(GDatabase *db, offset_t offset)
{
    ...
    gdbLockFreeBlockList(db, DB_READ_LOCK, LOC_DB_0121);
    block = gdbCacheGetBlockNoLock(db, offset);
    gdbUnlockFreeBlockList(db, LOC_DB_0122);
    return block;
}
...
{
    gdbLockFreeBlockList(db, DB_READ_LOCK, LOC_DB_0123);
    gdbCachePrintBlockNoLock(log, db);
    gdbUnlockFreeBlockList(db, LOC_DB_0124);
    return;
}
