...
btreeReadHeader(GdbBlock *block, const uint8_t *buffer, void *extra)
{
    ...
    MEM_CHECK(tree = (BTree *)SAFE_MALLOC(sizeof(BTree), LOC_BTREE_0037));
    ...
    BTREE_CRWLOCK_INIT(tree, LOC_BTREE_0038);
    return tree;
}
...
btreeWriteHeader(GdbBlock *block, uint8_t **buffer, uint32_t *size)
{
    ...
    *size = BTREE_HEADER_DATA_SIZE;
    ...
    MEM_CHECK(*buffer = (uint8_t *)SAFE_MALLOC(BTREE_HEADER_DATA_SIZE, LOC_BTREE_0039));
...
btreeCreateHeader(GdbBlock *block, void *extra)
{
    ...
    MEM_CHECK(tree = (BTree *)SAFE_MALLOC(sizeof(BTree), LOC_BTREE_0040));
    memset(tree, 0, sizeof(BTree));
    ...
    BTREE_CRWLOCK_INIT(tree, LOC_BTREE_0041);
...
btreeDestroyHeader(void *tree)
{
    if (NULL != tree)
    {
        BTREE_CRWLOCK_CLEAN((BTree *)tree, LOC_BTREE_0042);
        SAFE_FREE(tree, LOC_BTREE_0043);
    }
    return;
}
...
btreeSetRootNode(BTree *tree, offset_t offset)
{
    ...
    rawFileWrite(fp, __offset, &offset, sizeof(offset_t), 1, LOC_BTREE_0044);
}
...
btreeSetLeftLeaf(BTree *tree, offset_t offset)
{
    ...
    rawFileWrite(fp, __offset, &offset, sizeof(offset_t), 1, LOC_BTREE_0045);
}
...
btreeSetTreeSize(BTree *tree, uint32_t size)
{
    ...
    rawFileWrite(fp, __offset, &size, sizeof(uint32_t), 1, LOC_BTREE_0046);
}
...
btreeGetRootNode(BTree *tree)
{
    ...
    rawFileSeek(fp, block->offset + GDB_BLOCK_HEADER_SIZE + BTREE_ROOT_OFFSET, SEEK_SET);
    __offset = block->offset + GDB_BLOCK_HEADER_SIZE + BTREE_ROOT_OFFSET;
    if (rawFileRead(fp, __offset, &root_offset, sizeof(offset_t), 1, LOC_BTREE_0047) != 1)
    {
...
btreeGetLeftLeaf(BTree *tree)
{
    ...
    rawFileSeek(fp, block->offset + GDB_BLOCK_HEADER_SIZE + BTREE_LEFT_LEAF_OFFSET, SEEK_SET);
    __offset = block->offset + GDB_BLOCK_HEADER_SIZE + BTREE_LEFT_LEAF_OFFSET;
    if (rawFileRead(fp, __offset, &leaf_offset, sizeof(offset_t), 1, LOC_BTREE_0048) != 1)
    {
...
btreeGetTreeSize(BTree *tree)
{
    ...
    rawFileSeek(fp, block->offset + GDB_BLOCK_HEADER_SIZE + BTREE_SIZE_OFFSET, SEEK_SET);
    __offset = block->offset + GDB_BLOCK_HEADER_SIZE + BTREE_SIZE_OFFSET;
    if (rawFileRead(fp, __offset, &tree_size, sizeof(uint32_t), 1, LOC_BTREE_0049) != 1)
    {
...
void btreeDebug0(BTree *tree, const word_t location)
{
    ...
    if (rawFileRead(fp, __offset, &order, sizeof(uint8_t), 1, LOC_BTREE_0050) != 1)
    {
    ...
    __offset = block->offset + GDB_BLOCK_HEADER_SIZE + BTREE_SIZE_OFFSET;
    if (rawFileRead(fp, __offset, &size, sizeof(uint32_t), 1, LOC_BTREE_0051) != 1)
    {
    ...
    __offset = block->offset + GDB_BLOCK_HEADER_SIZE + BTREE_ROOT_OFFSET;
    if (rawFileRead(fp, __offset, &root, sizeof(offset_t), 1, LOC_BTREE_0052) != 1)
    {
    ...
    __offset = block->offset + GDB_BLOCK_HEADER_SIZE + BTREE_LEFT_LEAF_OFFSET;
    if (rawFileRead(fp, __offset, &leftLeaf, sizeof(offset_t), 1, LOC_BTREE_0053) != 1)
    {
    ...
    if (rawFileRead(fp, __offset, &freeBlockCount, sizeof(uint32_t), 1, LOC_BTREE_0054) != 1)
    {
