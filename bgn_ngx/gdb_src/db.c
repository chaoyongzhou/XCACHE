...
__setupDatabase(GDatabase *db)
{
    ...
    MEM_CHECK(db->openBlocks = (GdbBlock **)SAFE_MALLOC(db->openBlockSize * sizeof(GdbBlock *), LOC_DB_0068));
...
static uint8_t *__genIdxFileName(const uint8_t *root_path, const word_t table_id)
{
    ...
    MEM_CHECK(fidx_name = (uint8_t *)SAFE_MALLOC(len, LOC_DB_0069));
...
static uint8_t *__genDatFileName(const uint8_t *root_path, const word_t table_id)
{
    ...
    MEM_CHECK(fdat_name = (uint8_t *)SAFE_MALLOC(len, LOC_DB_0070));
...
{
    uint8_t *file_name;
    MEM_CHECK(file_name = (uint8_t *)SAFE_MALLOC(strlen((char *)root_path) + 1, LOC_DB_0071));
...
gdbMake(const uint8_t *root_path, const word_t table_id, const word_t cdfs_md_id, const int idx_fd, const int dat_fd)
{
    ...
    MEM_CHECK(db = (GDatabase *)SAFE_MALLOC(sizeof(GDatabase), LOC_DB_0072));
    memset(db, 0, sizeof(GDatabase));
    gdbInitlockFreeBlockList(db, LOC_DB_0073);
    ...
    SAFE_FREE(fidx_name, LOC_DB_0074);
    SAFE_FREE(fdat_name, LOC_DB_0075);
...
gdbOpen(const uint8_t *root_path, const word_t table_id, const word_t cdfs_md_id, const int flags)
{
    ...
    if(flags & O_RDWR)
    {
        ...
        SAFE_FREE(fidx_name, LOC_DB_0076);
    }
    ...
    MEM_CHECK(db = (GDatabase *)SAFE_MALLOC(sizeof(GDatabase), LOC_DB_0077));
    memset(db, 0, sizeof(GDatabase));
    gdbInitlockFreeBlockList(db, LOC_DB_0078);
...
gdbCreate(const uint8_t *root_path, const word_t table_id, const word_t cdfs_md_id)
{
    ...
    if(NULL == idxRawFile)
    {
        ...
        SAFE_FREE(fidx_name, LOC_DB_0082);
        return NULL;
    }
    SAFE_FREE(fidx_name, LOC_DB_0083);
    ...
    if(NULL == datRawFile)
    {
        ...
        SAFE_FREE(fdat_name, LOC_DB_0084);
        rawFileClose(idxRawFile);
        return NULL;
    }
    SAFE_FREE(fdat_name, LOC_DB_0085);

    MEM_CHECK(db = (GDatabase *)SAFE_MALLOC(sizeof(GDatabase), LOC_DB_0086));
    memset(db, 0, sizeof(GDatabase));
    gdbInitlockFreeBlockList(db, LOC_DB_0087);
...
gdbDestroy(GDatabase *db)
{
    ...
    SAFE_FREE(db->openBlocks, LOC_DB_0088);
    SAFE_FREE(db->filename, LOC_DB_0089);
...
gdbUnlink(const uint8_t *root_path, const word_t table_id)
{
    ...
    SAFE_FREE(fidx_name, LOC_DB_0091);
    SAFE_FREE(fdat_name, LOC_DB_0092);
...
gdbInsertKeyValue(GDatabase *db, const KeyValue *keyValue, const uint8_t replaceDup)
{
    ...
    if(NULL != db->datRawFile)
    {
        ...
        MEM_CHECK(kv  = kvNewHs(keyValue, LOC_DB_0093));
        kvPutHs(kv, keyValue);
    ...
    if(RAW_FILE_SUCC == result)
    {
        ...
        MEM_CHECK(key = keyNewHs(klen, LOC_DB_0095));
...
gdbDeleteKey(GDatabase *db, const uint8_t *key, int(* keyCompare)(const uint8_t *, const uint8_t *), offset_t *offset)
{
    ...
    BTREE_CRWLOCK_WRLOCK(db->mainTree, LOC_DB_0097);
    (*offset) = btreeDelete(db->mainTree, key);
    BTREE_CRWLOCK_UNLOCK(db->mainTree, LOC_DB_0098);
    if(0 == (*offset))
    {
        return GDB_ERROR;
    }
...
gdbFetchKey(const GDatabase *db, const offset_t offset, uint8_t **key, uint16_t *key_len)
{
    ...
    (*key) = (uint8_t *)SAFE_MALLOC(tlen, LOC_DB_0099);
    if(NULL == (*key))
    {
...
gdbFetchValue(const GDatabase *db, const offset_t offset, uint8_t **val, uint32_t *val_len)
{
    ...
    (*val) = (uint8_t *)SAFE_MALLOC(vlen, LOC_DB_0101);
    if(NULL == (*val))
    {
...
gdbFetchKV(const GDatabase *db, const offset_t offset, uint8_t **kv, uint32_t *kv_len)
{
    ...
    (*kv) = (uint8_t *)SAFE_MALLOC(dlen, LOC_DB_0103);
    if(NULL == (*kv))
    {
...
gdbKeyRegexScanKVOffset(const GDatabase *db, int (*key_regex)(const uint8_t *, pcre *, pcre *, pcre *), pcre *row_re, pcre *colf_re, pcre *colq_re, CMAP *kvoffset_map)
{
         ...
         offset = btreeGetNextOffset(trav))
    {
        ...
        if(0 != key_regex(key, row_re, colf_re, colq_re))
        {
            ...
            offset_word = offset;
            cmap_add(kvoffset_map, (void *)offset_word, (void *)kv_bytes, LOC_DB_0108);
            continue;
        }
...
gdbKVRegexScanKVOffset(const GDatabase *db, int (*kv_regex)(const uint8_t *, pcre *, pcre *, pcre *, pcre *), pcre *row_re, pcre *colf_re, pcre *colq_re, pcre *val_re, CMAP *kvoffset_map)
{
         ...
         offset = btreeGetNextOffset(trav))
    {
        ...
        if(0 != kv_regex(key, row_re, colf_re, colq_re, val_re))
        {
            ...
            offset_word = offset;
            cmap_add(kvoffset_map, (void *)offset_word, (void *)kv_bytes, LOC_DB_0113);
            continue;
        }
